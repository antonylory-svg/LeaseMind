...
9. `03_ARCHITECTURE/ai-manager/LeaseMind_AI_MANAGER_ARCHITECTURE_v1.0.md`, версия 1.0, статус Approved.
...
### 3.1. Нормативная пользовательская последовательность
...
Эта последовательность является нормативной. `HOMEPAGE.md` и `FIRST_ANALYSIS.md` синхронизированы с ней; точный контракт результата Analysis определён в `ANALYSIS_SNAPSHOT.md`.
...
| Термин | Значение |
| --- | --- |
| Техническое задание / `TechnicalAssignment` | Версионированный application-level payload, который валидируется и сохраняется в `Property` либо `TenantRequest`. Не является отдельной доменной сущностью из `03_ARCHITECTURE/Data_Model.md`. |
...
1. `TechnicalAssignment` — входной контракт, а не новая сущность `03_ARCHITECTURE/Data_Model.md`.
...
| --- | --- | --- | --- | --- |
| 3.1, 11 | Зафиксировать pre-launch Analysis в утверждённой последовательности и отдельно сохранить post-launch refresh ≤15 минут | PRODUCT; редакционная синхронизация `HOMEPAGE.md`/`FIRST_ANALYSIS.md` — PRODUCT | Blocking | Исправлено; связанные документы синхронизированы, точный контракт вынесен в `ANALYSIS_SNAPSHOT.md` |
