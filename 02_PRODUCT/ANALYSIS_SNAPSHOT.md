...
6. `02_PRODUCT/CAMPAIGN_OUTCOMES.md` — PRODUCT-источник истины для бизнес-исходов (`business_outcome`) Campaign и определения «созревшей» (mature) Campaign (раздел 9.8).
...
Термины (согласованы с `02_PRODUCT/CAMPAIGN_OUTCOMES.md` — PRODUCT-источником истины для бизнес-исходов Campaign и определения «созревшей» Campaign):

- «Созревшая кампания» — Campaign с зафиксированным effective terminal `business_outcome`: одним из пяти outcome codes `success_via_leasemind`, `success_independently`, `success_via_broker`, `cancelled`, `expired` (`CAMPAIGN_OUTCOMES.md`, раздел 5). `Paused` не является исходом и Campaign в этом статусе не считается «созревшей» (`CO-C-004`). Campaign с lifecycle status `Completed` или `Failed`, но без явно зафиксированного `business_outcome` (например, существующая до Sprint 6 Campaign Outcomes), также не считается «созревшей» и не получает outcome автоматически или задним числом (`CO-C-028`).
- «Событие» — созревшая кампания с `business_outcome`, классифицированным как сделка: `success_via_leasemind`, `success_independently` или `success_via_broker`.
- «Несобытие» — созревшая кампания с `business_outcome`, классифицированным как отсутствие сделки: `cancelled` или `expired`. `Paused` не входит ни в события, ни в несобытия.
...
5. **Исключение synthetic-only записей**: записи с runtime mode `synthetic` полностью исключаются из обучения, валидации и любых продуктовых выводов о готовности порогов; в подсчёт по пунктам 1–4 входят только записи с runtime mode, отличным от `synthetic`, и подтверждённым реальным terminal outcome (см. также `CAMPAIGN_OUTCOMES.md`, `CO-C-019`).
