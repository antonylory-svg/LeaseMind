...
**Reviewed repository commit:** `13ef3740ed566e8df9125aa4ba4c71a7ead59770`
...
> **Temporal hygiene note for Safe Presentation portion of Wave 2 row:** встроенное описание Wave 2D/§5.5 отражает исторический registry-only checkpoint. Текущее состояние задаёт более поздний §5.5.1: `XFR-D-072 v1.0` разрешает только `PARTIALLY_RESOLVED_BOUNDARY` owner/approver/evidence roles, default-deny, independent-row completeness, registry isolation, minimum qualitative evidence prerequisites, joint combination-risk/non-compensation и no-automatic-authorization semantics. Actual allowlist rows/fields/transformations/values/evidence, policy/runtime/implementation остаются `OPEN`; `XFR-D-073` registry identity не переписывается.
...
| №6 Safe Presentation allowlist | `PRODUCT + LEGAL` | `XFR-D-072` | Governance/evidence prerequisites `PARTIALLY_RESOLVED_BOUNDARY`; actual rows/fields/transformations/values, `XFR-D-074`, `XFR-D-M3`, `XFR-D-075`–`084`, evidence/policy/runtime remain open | `PARTIALLY_RESOLVED_BOUNDARY` |
