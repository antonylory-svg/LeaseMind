...
Canonical IDs/роли в §4.5 (Scoring Policy crosswalk) не изменены. Ниже — честный overlay статуса трёх MSP-строк (`MSP-10`, `MSP-15`, `MSP-17`), разрешённых Wave 2F qualitative governance decisions, а также статус двух смежных строк (`MSP-16`, `MSP-18`), не получивших отдельный record.
...
`MSP-16` (`XFR-D-027`, owner конкретного шага Architecture §30.3) остаётся полностью `OPEN` — ни один прочитанный источник не устанавливает owner этого конкретного шага напрямую; отдельный decision record не создан этим Wave. `MSP-18` (`XFR-D-029`, anti-conflation guard «technical writer ≠ policy owner») уже задокументирован Scoring Policy §2/§12 как established principle; отдельный decision record для него не требуется и не создан.
...
- Scoring Policy: decision row №10 — `RESOLVED_QUALITATIVE_BOUNDARY` (`XFR-D-023`), decision row №15 — `RESOLVED_EVIDENCE_BOUNDARY` (`XFR-D-026`), decision row №17 — `PARTIALLY_RESOLVED_BOUNDARY` (`XFR-D-028`, только internal ownership; external granularity остаётся open); decision row №16 (`XFR-D-027`) остаётся полностью `OPEN`; decision row №18 (`XFR-D-029`) — established guard, отдельный record не требуется;
- этот inventory: Wave 1, Wave 2A, Wave 2B, Wave 2C, Wave 2D, Wave 2E и Wave 2F status overlays (§5.1, §5.2, §5.3, §5.4, §5.5, §5.6, §5.7).

Следующий узкий package — оставшаяся qualitative policy semantics: Scoring governance и Scoring Policy §12 НЕ завершены Wave 2F (non-exhaustive: segment-override evidence, ranking/diversification boundaries, Priority Score governance, `XFR-D-027`/§30.3 step owner остаётся `OPEN`); Evaluation-specific compatibility; Safe Presentation field allowlist и combination-risk algorithm и т.п. — без numeric values и без runtime design.
