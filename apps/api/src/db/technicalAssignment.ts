...
/**
 * Creates or updates one Technical Assignment draft. Idempotency key scope
 * (ADR-0008 section 1): the same idempotency_key always targets the same
 * draft row; an identical resubmitted payload is a no-op (no revision
 * bump); a changed payload updates the same draft and bumps revision;
 * reusing the key under the other scenario is rejected without any write
...
export async function saveTechnicalAssignmentDraft(
  ...
  try {
    ...
    const existingRaw = await findRowByIdempotencyKey(client, table, idColumn, input.idempotencyKey);
    const existing = existingRaw ? coerceStoredRow(existingRaw, specs) : null;
    ...
    let revision: number = existing ? (existing.revision as number) : 1;
    if (existing) {
      const changed = specs.some(spec => !normalizedEquals(normalized[spec.fieldId], (existing[spec.fieldId] as NormalizedValue) ?? null));
    ...
    // particular save call -- from the value already on the existing row,
    // never by querying the protected table.
    ...
    if (isNew) {
      ...
      await client.query(`UPDATE ${table} SET ${setClauses.join(', ')} WHERE ${idColumn} = $${updateValues.length + 1}`, [
        ...updateValues,
