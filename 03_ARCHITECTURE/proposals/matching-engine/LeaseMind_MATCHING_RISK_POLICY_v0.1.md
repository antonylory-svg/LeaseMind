...
Список не закрывается произвольно. В каждой из 14 строк owner либо имеет прямую source-ссылку (№4 — Architecture §37 №8; №14 — Architecture §37 №11 и отдельно Feature Schema открытое решение №6, разведены, не объединены в один owner), либо явно помечен как candidate assignment (№1, 3, 5, 6, 9, 10, 11) — источник не назначает owner этого решения напрямую. №2 — approved decision-specific owner assignment `XFR-D-048 v1.0`; №7 — approved decision-specific assignment `XFR-D-052 v1.0`; №8 — approved decision-specific assignment `XFR-D-053 v1.0`; №12 — approved decision-specific interface assignment `XFR-D-055 v1.0`; №13 — approved decision-specific assignment `XFR-D-056 v1.0`. Эти assignments сознательно не изменяют artifact owners Risk/Qualification, source writers или source-owned `XFR-D-M2` threshold authority.
