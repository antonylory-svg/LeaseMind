...
Список не закрывается произвольно. В каждой из 14 строк owner либо имеет прямую source-ссылку (№4 — Architecture §37 №8; №14 — Architecture §37 №11 и отдельно Feature Schema открытое решение №6, разведены, не объединены в один owner), либо явно помечен как candidate assignment (№1, 3, 5, 6, 8, 9, 10, 11, 13) — источник не назначает owner этого решения напрямую. №2 — approved decision-specific owner assignment `XFR-D-048 v1.0`; №7 — approved decision-specific assignment `XFR-D-052 v1.0`; №12 — approved decision-specific interface assignment `XFR-D-055 v1.0`. Эти assignments сознательно не изменяют artifact owners Risk/Qualification или source-owned `XFR-D-M2` threshold authority.
...
- не содержит ни одного численного Risk weight/threshold/TTL/calibration target, ни одного выбранного numeric aggregation algorithm, ни одного public/runtime enum или reason-code каталога; qualitative aggregation/non-compensation boundary утверждена `XFR-D-048 v1.0`, Risk reason-reference governance boundary частично разрешена `XFR-D-052 v1.0`, а qualitative Risk→Qualification governance/interface boundary — `XFR-D-055 v1.0`; exact namespace/codes/values/order/severity/mappings/compatibility/fallback/TTL/hash fields/trigger/schema/carrier/data/evidence/RBAC/policies/manifest/runtime/implementation остаются `OPEN`, и сам Proposal остаётся `Proposal`, не `APPROVED`;
