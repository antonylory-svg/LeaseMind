...
Human-approved governance decisions `LeaseMind_MATCHING_DECISION_XFR-D-048_v1.0.md` (Risk aggregation qualitative model and non-compensation boundary), `LeaseMind_MATCHING_DECISION_XFR-D-052_v1.0.md` (Risk reason-reference governance boundary), `LeaseMind_MATCHING_DECISION_XFR-D-053_v1.0.md` (Risk reviewer-authority/queue/Decision-linkage governance boundary), `LeaseMind_MATCHING_DECISION_XFR-D-055_v1.0.md` (Risk output → Qualification interface governance boundary) и `LeaseMind_MATCHING_DECISION_XFR-D-056_v1.0.md` (duplication-detection governance/authority boundary) обязательны для соответствующих status overlays §5/§9/§10/§11/§13/§14/§15 ниже. Их approval не переводит этот Proposal в `APPROVED`: `XFR-D-052`/`XFR-D-053`/`XFR-D-055`/`XFR-D-056` разрешают только `PARTIALLY_RESOLVED_BOUNDARY`; exact namespace/codes/values/order/severity/mappings/compatibility/fallback/TTL, trigger, entity taxonomy/keys, duplication algorithm/threshold/evidence/reviewer appointments/RBAC/queue/SLA/outcomes/Decision schema/appeal, numeric content, runtime representation/carrier, policies и implementation остаются `OPEN`.

**Связанные документы:** `LeaseMind_MATCHING_ENGINE_ARCHITECTURE_v1.1.md`, `LeaseMind_MATCHING_DATA_CONTRACTS_v1.0.md` (контрактные/replay границы — проверено: Risk-специфичных names/enums, generic Risk-review queue/carrier и entity-duplication carrier там нет; dispute/previous-contact-specific decision structures не являются generic Risk-review authority; event/inbox/idempotency deduplication не является entity resolution), `LeaseMind_MATCHING_FEATURE_SCHEMA_v0.1.md` (Proposal-зависимость, source facts/evidence), `LeaseMind_MATCHING_EVALUATION_PLAN_v0.1.md` (Proposal-зависимость, dataset/metric/threshold-search procedure evidence, не owner финальных Risk thresholds), `LeaseMind_MATCHING_QUALIFICATION_POLICY_v0.1.md` (Proposal-зависимость, final routing boundary; `XFR-D-041` reviewer queue/authority остаётся distinct), `LeaseMind_MATCHING_DECISION_XFR-D-048_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-052_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-053_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-055_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-056_v1.0.md`, `02_PRODUCT/CAMPAIGN_OUTCOMES.md`, `02_PRODUCT/ANALYSIS_SNAPSHOT.md`, `05_DEVELOPMENT/matching-engine/reviews/LeaseMind_DEVELOPMENT_REVIEW_MATCHING_ENGINE_v1.1_EIGHTH.md` (только DEVELOPMENT evidence, не источник новых архитектурных решений).
...
Список не закрывается произвольно. В каждой из 14 строк owner либо имеет прямую source-ссылку (№4 — Architecture §37 №8; №14 — Architecture §37 №11 и отдельно Feature Schema открытое решение №6, разведены, не объединены в один owner), либо явно помечен как candidate assignment (№1, 3, 5, 6, 9, 10) — источник не назначает owner этого решения напрямую. №2 — approved decision-specific owner assignment `XFR-D-048 v1.0`; №7 — approved decision-specific assignment `XFR-D-052 v1.0`; №8 — approved decision-specific assignment `XFR-D-053 v1.0`; №11 — human-approved candidate-derived, explicitly not source-normative assignment `XFR-D-M4 v1.0`; №12 — approved decision-specific interface assignment `XFR-D-055 v1.0`; №13 — approved decision-specific assignment `XFR-D-056 v1.0`. Эти assignments сознательно не изменяют artifact owners Evaluation/Risk/Qualification/Scoring, source writers или source-owned `XFR-D-M2` threshold authority.
...
**Given:** утверждённый qualitative aggregation boundary `XFR-D-048 v1.0` (multi-component representation + conditional non-compensation invariant для отдельно классифицированных critical категорий). **When:** формируется итоговый Risk output. **Then:** общий audit-bundle §33 (`SOURCE_NORMATIVE`) уже требует сохранения активных критериев/причин исключения/конфликтов для любого расчёта Matching Engine — не специфично для Risk; per-category Risk detail остаётся non-compensating и видимым по `XFR-D-048`. `XFR-D-052 v1.0` разрешает только `PARTIALLY_RESOLVED_BOUNDARY` internal opaque reason-reference governance/version-provenance/fail-closed semantics и не позволяет summary/reference скрыть components; `XFR-D-055 v1.0` разрешает только `PARTIALLY_RESOLVED_BOUNDARY` read-only hand-off и fail-closed interface semantics. Для duplication `XFR-D-056 v1.0` не классифицирует possible или confirmed duplicate как critical: `XFR-D-048` non-compensation применяется к duplicate Risk component только после independently approved critical classification. Численная формула, runtime representation, critical-category mapping, exact reason namespace/values/mappings, exact Risk→Qualification trigger/mapping/carrier остаются `OPEN` под `XFR-D-047`/`XFR-D-049`/`XFR-D-050`/`XFR-D-051`/`XFR-D-052`/`XFR-D-053`/`XFR-D-055`/`XFR-D-M2`. Authoritative dependency split зафиксирован в XFR-D-048 §9, XFR-D-052 §§4–6, XFR-D-055 §5, XFR-D-056 §§3–5 и Inventory §5.6–§5.6.4.
...
**Given:** Risk Policy candidate threshold найден через `MATCHING_EVALUATION_PLAN` или завершён replay. **When:** evaluation/replay завершён. **Then:** runtime rules не меняются автоматически (§34.4: «Автоматическое изменение продуктивных правил по результатам обучения — 0 случаев»; согласовано с Evaluation Plan `MEP-C-014/018`); `XFR-D-M4 v1.0` также сохраняет historical Match Result immutable и не создаёт automatic policy/model/runtime/gate action.
