...
- **Статус:** `SOURCE_NORMATIVE` — семейство и требование future approved threshold названы явно; governance/evidence boundary — `PARTIALLY_RESOLVED_BOUNDARY` по `XFR-D-063 v1.0`; все targets, `K`, exact metric/relevance definitions, numerator/denominator/aggregation, uncertainty и statistical comparison остаются `OPEN_BLOCKED_PENDING_DECISION`.
