...
Human-approved `LeaseMind_MATCHING_DECISION_XFR-D-M4_v1.0.md` independently gives merged `EP-10 + MRP-11 + MQP-15 + MSP-14` only `PARTIALLY_RESOLVED_BOUNDARY`: all four source keys remain `PRIMARY_MERGED_MEMBER`; governance owner `DEVELOPMENT + AI` is candidate-derived and not `SOURCE_NORMATIVE`; mandatory approvers are `Chief AI Architect + PRODUCT + LEGAL`; evidence/technical owner `DEVELOPMENT + AI` has no unilateral authority; Evaluation, Risk, Qualification and Scoring artifact owners remain separate under Architecture §52. Exact deterministic replay remains exact-only and severity-1 on mismatch; recorded and bounded probabilistic replay remain distinct; probabilistic output is advisory only. Every exact tolerance/invariant/data/statistical/policy/production/carrier/runtime/implementation content remains `OPEN`, and no gate is changed.

**Related replay decision:** `LeaseMind_MATCHING_DECISION_XFR-D-M4_v1.0.md`.

**Current merged row №9 overlay:** human-approved `XFR-D-M3 v1.0` gives `EP-09` only `PARTIALLY_RESOLVED_BOUNDARY`; canonical `FS-07 + EP-09 + MRP-10 + SPP-04 → XFR-D-M3`, with all four source keys remaining `PRIMARY_MERGED_MEMBER`. Governance owner is `PRODUCT + LEGAL`, human-approved candidate-derived and not `SOURCE_NORMATIVE` for the whole merge; `DEVELOPMENT` is measurability-only. Artifact owners remain Feature `PRODUCT + LEGAL + AI`, Evaluation `AI + DEVELOPMENT`, Risk `Chief AI Architect + LEGAL`, Safe Presentation `PRODUCT + LEGAL`; mandatory approvers are `Chief AI Architect + AI + DEVELOPMENT`; evidence/technical-procedure owner is `AI + DEVELOPMENT` without unilateral authority; any future exact change requires all five functions on the same version/hash and immutable evidence package. Only role separation, four-context separation, closed version/hash-bound candidate discipline, affected-use fail-closed, non-compensation, source-normative deny preservation, historical binding, no-automatic-action and qualitative evidence prerequisites are approved. Dataset irreversible de-identification, Evaluation privacy/small-cell diagnostics, Risk signal and Safe Presentation evidence remain distinct with no cross-context authorization; pseudonymization is not anonymization; privacy small-cell is not statistical power/coverage; exact address, coordinates and high-risk identifying combinations remain denied. Exact method/cohort/quasi-ID/adversary/linkage/numerator/denominator/counting/threshold/comparator/tolerance/statistics/data/evidence/policy/production/schema/runtime/implementation remain `OPEN`; independent decisions and all three gates remain unchanged/`BLOCKED`.

Human-approved `LeaseMind_MATCHING_DECISION_XFR-D-045_v1.0.md` независимо разрешает только `PARTIALLY_RESOLVED_BOUNDARY` governance/evidence-prerequisite части Qualification Policy row 19: owner `Chief AI Architect + AI`, mandatory approvers `PRODUCT + LEGAL + DEVELOPMENT`, evidence/technical-procedure owner `AI + DEVELOPMENT` без unilateral authority; three-family separation/non-compensation, baseline-first, frozen-manifest, tuning/final isolation и fail-closed prerequisite discipline обязательны. `XFR-F1` Qualification-specific metric-family content, exact/numeric/statistical content, dataset/run/evidence package/results/verdict, production applicability, policy/manifest, runtime и implementation остаются `OPEN`; этот record не добавляет metric family в настоящий Plan и не изменяет `XFR-D-057`–`071`.

Human-approved governance decisions `LeaseMind_MATCHING_DECISION_XFR-D-058_v1.1.md` (fail-closed boundary + exact human adjudication governance procedure, supersedes v1.0), `LeaseMind_MATCHING_DECISION_XFR-D-059_v1.1.md` (conservative connected-component grouping/split-isolation policy, supersedes v1.0), `LeaseMind_MATCHING_DECISION_XFR-D-060_v1.0.md` (conservative correction-history exclusion at a new freeze), `LeaseMind_MATCHING_DECISION_XFR-D-061_v1.0.md` (false-exclusion maximum governance owner and evidence-prerequisite boundary), `LeaseMind_MATCHING_DECISION_XFR-D-062_v1.0.md` (dataset split/allocation governance and reproducibility boundary), `LeaseMind_MATCHING_DECISION_XFR-D-063_v1.0.md` (numeric metric-target governance owner, metric-family separation and evidence-prerequisite boundary), `LeaseMind_MATCHING_DECISION_XFR-D-064_v1.0.md` (segment/bias/proxy diagnostic dataset-coverage governance owner and evidence-prerequisite boundary), `LeaseMind_MATCHING_DECISION_XFR-D-065_v1.0.md` (drift-monitoring governance owner, artifact-separation and qualitative evidence boundary), `LeaseMind_MATCHING_DECISION_XFR-D-066_v1.0.md` (exact cross-functional artifact-approval procedure, without actual approval), `LeaseMind_MATCHING_DECISION_XFR-D-068_v1.0.md` (fairness governance owner, diagnostic-vs-legal non-conflation and evidence-prerequisite boundary), `LeaseMind_MATCHING_DECISION_XFR-D-069_v1.0.md` (qualitative `unknown`/`abstention` terminology boundary), `LeaseMind_MATCHING_DECISION_XFR-D-070_v1.0.md` (threshold-search statistical-comparison governance, pre-registration and qualitative evidence boundary) и `LeaseMind_MATCHING_DECISION_XFR-D-071_v1.0.md` (post-freeze correction impact-review governance and immutable-history/no-silent-reuse boundary) обязательны для соответствующих граней ниже (§4, §5.4–§5.5, §6.1–§6.4, §6.8, §7, §8–§10, §11, `MEP-C-002`, `MEP-C-003`, `MEP-C-005`, `MEP-C-010`–`MEP-C-014`). Их approval не переводит этот Proposal в `APPROVED`, не утверждает dataset, numeric size/ratios/tolerance, exact allocation algorithm/seed value, runtime graph/manifest carrier, production use, numeric false-exclusion maximum, numeric metric target, `K`, segment universe, protected/proxy classification, lawful-basis determination, exact fairness standard/metric/comparator/threshold/statistical method, exact/numeric threshold-search statistical procedure, drift taxonomy/metric/baseline/window/trigger, production operational artifact, exact correction notification/disposition mechanism или exact metric/statistical content, не создаёт actual artifact-approval record/Controlled Artifact Manifest entry и не вводит runtime enum, routing mapping, alert/action mapping или численные triggers.

**Связанные документы:** `LeaseMind_MATCHING_ENGINE_ARCHITECTURE_v1.1.md`, `LeaseMind_MATCHING_DATA_CONTRACTS_v1.0.md` (только контрактные/версионные/replay-границы), `LeaseMind_MATCHING_FEATURE_SCHEMA_v0.1.md` (Proposal-зависимость, не утверждённый runtime contract), `LeaseMind_MATCHING_DECISION_XFR-D-057_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-058_v1.1.md` (supersedes `XFR-D-058 v1.0`), `LeaseMind_MATCHING_DECISION_XFR-D-059_v1.1.md` (supersedes `XFR-D-059 v1.0`), `LeaseMind_MATCHING_DECISION_XFR-D-060_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-061_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-062_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-063_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-064_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-065_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-066_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-067_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-068_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-069_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-070_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-071_v1.0.md`, `LeaseMind_MATCHING_RISK_POLICY_v0.1.md` (только §13 открытое решение №9, protected/proxy classification catalog), `02_PRODUCT/CAMPAIGN_TECHNICAL_ASSIGNMENT.md`, `02_PRODUCT/ANALYSIS_SNAPSHOT.md`, `02_PRODUCT/CAMPAIGN_OUTCOMES.md`, `05_DEVELOPMENT/matching-engine/reviews/LeaseMind_DEVELOPMENT_REVIEW_MATCHING_ENGINE_v1.1_EIGHTH.md`.
...
Для будущего Qualification-threshold evidence package действует отдельный decision-specific split `XFR-D-045 v1.0`: governance owner `Chief AI Architect + AI`, mandatory approvers `PRODUCT + LEGAL + DEVELOPMENT`, evidence/technical-procedure owner `AI + DEVELOPMENT` без unilateral authority. Он не меняет artifact ownership настоящего Proposal и не назначает owners фактических `XFR-D-034`/`035`/`036` thresholds.
...
- корректировки outcome не переписывают уже frozen run задним числом — human-approved `XFR-D-060 v1.0`; `XFR-D-071 v1.0` разрешает только accepted-correction trigger, source-authoritative discovery, immutable-history/append-only impact evidence и no-silent-reuse boundary для `FROZEN`/`EXECUTED`/`REVIEWED` run. Exact notification/disposition schema, per-run quorum/RBAC/SLA, lifecycle/verdict mapping, remediation/rollback и runtime carrier остаются `OPEN_BLOCKED_PENDING_DECISION`.
...
- **Frozen historical evaluation run не переписывается после correction.** Для нового freeze исправленная Campaign исключается; `XFR-D-071 v1.0` разрешает qualitative post-freeze impact-review governance boundary, но exact notification/disposition/runtime mechanism остаётся `OPEN`.
...
- post-freeze correction не переписывает historical run; `XFR-D-071 v1.0` требует append-only impact evidence, positive proof для `demonstrably unaffected`, новый versioned cycle при affected impact и fail-closed block при unresolved impact, оставляя exact synchronization mechanism `OPEN`.
...
- `XFR-D-060 v1.0` — `RESOLVED_CONSERVATIVE_CORRECTION_HISTORY_EXCLUSION_BOUNDARY`: option B исключает исправленную Campaign и все её outcome-derived labels из нового freeze без удаления canonical linkage; отсутствие correction остаётся только необходимым, не достаточным условием eligibility. `XFR-D-071 v1.0` разрешает qualitative post-freeze correction impact-review boundary; exact notification/disposition workflow, source-history controls, manifest/runtime carrier, selection-bias quantitative boundary и production-data prerequisites остаются `OPEN_BLOCKED_PENDING_DECISION` (§11 пункты 4/17).
- `XFR-D-061 v1.0` — `PARTIALLY_RESOLVED_BOUNDARY`: governance owner будущего numeric false-exclusion maximum — `Chief AI Architect + AI`, mandatory approvers — `PRODUCT + LEGAL + DEVELOPMENT`, evidence-procedure owner — `AI + DEVELOPMENT`; подготовка evidence не равна unilateral governance approval. Boundary относится только к подтверждённой успешной паре, ошибочно исключённой Hard Filter на eligible verified labeling. Numeric maximum, baseline, numerator/denominator, confidence/uncertainty method, aggregation window и exact statistical comparison остаются `OPEN_BLOCKED_PENDING_DECISION`; `XFR-D-057`–`XFR-D-060` обязательны, qualitative boundary `XFR-D-062 v1.0` применяется, но его numeric size/ratios/exact algorithm/seed policy/value вместе с `XFR-D-064`/`XFR-D-068` и exact/numeric statistical contents `XFR-D-070` остаются открытыми там, где применимы. Qualitative governance/pre-registration/tuning-final boundary `XFR-D-070 v1.0` применяется, но не подменяет metric-specific contents. `XFR-D-063 v1.0` разрешает отдельную metric-target governance/evidence boundary, но не подменяет false-exclusion maximum. Два отдельных Architecture §34.1 правила `0%` для unknown-as-negative и process-failure-as-negative не получают tolerance, waiver, aggregation или компенсацию.
- `XFR-D-063 v1.0` — `PARTIALLY_RESOLVED_BOUNDARY`: governance owner будущих in-scope targets `Precision@K`/`Recall@K`/`NDCG@K`/Confidence Score calibration/upper-result diversification — `Chief AI Architect + AI`, mandatory approvers — `PRODUCT + LEGAL + DEVELOPMENT`, evidence-procedure owner — `AI + DEVELOPMENT`; подготовка evidence не равна unilateral approval. `XFR-D-057`–`XFR-D-062` остаются mandatory prerequisites. Metric families не компенсируют друг друга; baseline измеряется до approved target; tuning evidence отделено от untouched final evidence. Qualitative pre-registration, comparison compatibility, full-reporting/non-compensation and no-automatic-action boundary `XFR-D-070 v1.0` применяется. Все numeric values, `K`, metric/relevance definitions, numerator/denominator/aggregation, calibration/diversification methods, uncertainty/confidence и exact/numeric statistical comparison остаются `OPEN_BLOCKED_PENDING_DECISION`; Risk Score/human-review, false-exclusion, rank/replay, critical-risk/Qualification, segment/fairness и runtime boundaries не подменяются.
...
- **Diversification boundary:** Architecture §34.2 отдельно называет качество диверсификации верхней выдачи; `XFR-D-063 v1.0` включает governance/evidence boundary будущего target, но не задаёт definition, dimension, formula, denominator, method или value.
- **Prohibited inference:** без размеченной тестовой выборки метрика не заключает о готовности к релизу.
- **Owner финального threshold:** Architecture §34.2 требует, чтобы точные пороги утверждались после создания размеченной тестовой выборки («До этого они измеряются как baseline и не подменяются произвольными значениями», дословно), но не называет owner напрямую; `XFR-D-063 v1.0` human-approved назначает governance owner `Chief AI Architect + AI`, mandatory approvers `PRODUCT + LEGAL + DEVELOPMENT` и evidence-procedure owner `AI + DEVELOPMENT` без unilateral approval authority.
- **Статус:** `SOURCE_NORMATIVE` — семейство и требование future approved threshold названы явно; governance/evidence boundary — `PARTIALLY_RESOLVED_BOUNDARY` по `XFR-D-063 v1.0`; qualitative statistical-comparison governance/pre-registration boundary — `PARTIALLY_RESOLVED_BOUNDARY` по `XFR-D-070 v1.0`; все targets, `K`, exact metric/relevance definitions, numerator/denominator/aggregation, uncertainty и exact/numeric statistical comparison остаются `OPEN_BLOCKED_PENDING_DECISION`.
...
- **Owner финального threshold (роль, не артефакт):** для Risk Score human-review thresholds — `AI + LEGAL`, Architecture §37 вопрос №8, `SOURCE_NORMATIVE`; они не входят в `XFR-D-063`. Для Confidence Score calibration target прямой источник owner не назначает; `XFR-D-063 v1.0` human-approved назначает governance owner `Chief AI Architect + AI`, mandatory approvers `PRODUCT + LEGAL + DEVELOPMENT` и evidence-procedure owner `AI + DEVELOPMENT` без unilateral approval authority. Выбранные значения фиксируются в артефактах `MATCHING_RISK_POLICY`/`MATCHING_SCORING_POLICY` соответственно — сами артефакты не являются owner-ролью.
- **Статус:** `SOURCE_NORMATIVE` — факторы Confidence Score названы явно (§16); governance/evidence boundary target'а — `PARTIALLY_RESOLVED_BOUNDARY` по `XFR-D-063 v1.0`; qualitative statistical-comparison governance/pre-registration boundary — `PARTIALLY_RESOLVED_BOUNDARY` по `XFR-D-070 v1.0`, но численная калибровка/target, exact metric/method/denominator/uncertainty и exact/numeric statistical comparison остаются `OPEN_BLOCKED_PENDING_DECISION`; qualitative terminology `unknown`/`abstention` резолвлена human-approved `XFR-D-069 v1.0`, но exact triggers, runtime mapping и routing остаются `OPEN_BLOCKED_PENDING_DECISION` (§11, пункт 15), новый runtime enum этим документом не вводится.

**Qualification Gate evidence gap — не новая metric family.** `XFR-D-045 v1.0` признаёт `XFR-F1`: настоящий registry не содержит отдельной Qualification-specific metric family для minimum mutual-fit threshold, Confidence routing cutoff или minimum critical-data completeness threshold/rule. Эти три условия образуют три отдельные evidence families без aggregate/cross-family compensation. Mutual-fit зависит от отдельно approved Scoring/Mutual Aggregate measurable object; Confidence calibration `XFR-D-063` является prerequisite input, но не cutoff; completeness отличается от per-feature `required_evidence_level`; `XFR-D-061` stage-3 false-exclusion и `XFR-D-M2` Risk→routing не являются substitutes. Governance/evidence-prerequisite boundary имеет `PARTIALLY_RESOLVED_BOUNDARY`, но `XFR-F1` content и все metric definitions/formulas/numerators/denominators, baselines/targets/thresholds/tolerances, sample/split/CI/window/test/uncertainty methods, dataset/run/results/verdict и production applicability остаются `OPEN_BLOCKED_PENDING_DECISION`.
...
- **Allowed use:** deterministic path допускает только exact replay (hashes/scores/ranking/reasons/package hash идентичны), без tolerance; для внешнего вероятностного компонента recorded replay по сохранённому неперсональному response artifact и bounded replay остаются разными режимами, причём bounded use требует отдельно approved exact candidate.
- **Prohibited inference:** вероятностный/недетерминированный компонент не проходит Matching Qualification Gate самостоятельно и остаётся advisory signal до human-confirmed deterministic rule (Architecture §49).
- **Governance owner:** `DEVELOPMENT + AI` — human-approved candidate-derived assignment `XFR-D-M4 v1.0`, не `SOURCE_NORMATIVE`; mandatory approvers `Chief AI Architect + PRODUCT + LEGAL`; evidence/technical owner `DEVELOPMENT + AI` не имеет unilateral authority. Artifact owner этого Evaluation Plan отдельно остаётся `AI + DEVELOPMENT` по Architecture §52.
- **Статус:** deterministic exact-replay mismatch остаётся severity-1 defect и блокирует соответствующую версию правил без epsilon/rounding/aggregate compensation (`SOURCE_NORMATIVE`, Architecture §49). `XFR-D-M4 v1.0` разрешает только `PARTIALLY_RESOLVED_BOUNDARY` для semantic separation, closed immutable version/hash-bound candidate discipline, preregistration, frozen allocation, tuning/final isolation, compatible comparison, separate reporting, affected-use fail-closed, non-compensation, historical immutability и no-automatic-action safeguards. Every exact tolerance value/range/unit/error metric/direction, absolute/relative/per-component/aggregate treatment, reason-code invariant catalog, probabilistic eligibility, recorded-artifact schema/retention/provenance, dataset/splits/sample/metrics/targets/statistics, representation/precision/rounding, policy/manifest approval, production applicability, carrier/runtime/monitoring/rollback/implementation remains `OPEN_BLOCKED_PENDING_DECISION`.
...
- **Owner финального threshold:** governance owner будущего fairness diagnostic framework/legal fairness standard — `LEGAL + PRODUCT` по human-approved `XFR-D-068 v1.0`; mandatory approvers — `Chief AI Architect + AI + DEVELOPMENT`; evidence-procedure owner — `AI + DEVELOPMENT`, без unilateral legal/product approval authority. Governance owner будущей **dataset-coverage sufficiency** policy (отдельный вопрос) утверждён `XFR-D-064 v1.0`: `PRODUCT + LEGAL`; mandatory approvers `Chief AI Architect + AI + DEVELOPMENT`; evidence-procedure owner `AI + DEVELOPMENT`.
- **Статус:** `SOURCE_NORMATIVE` для обязательности проверки по Architecture §30.3 п.4. Fairness governance/evidence boundary — `PARTIALLY_RESOLVED_BOUNDARY` по `XFR-D-068 v1.0`: diagnostic signal не является automatic legal verdict/causation claim; отсутствие disparity или labels не доказывает fairness/readiness; aggregate не компенсирует adverse/insufficient segment/intersection evidence; diagnostic-only sensitive-dimension use не передаёт authority в model/scoring/routing/runtime. Exact fairness doctrine, protected/proxy classification/lawful basis, population/segment/intersection, outcome/comparator, metric/numerator/denominator, threshold/tolerance, aggregation/weighting, uncertainty/statistics/causal method/remediation остаются `OPEN_BLOCKED_PENDING_DECISION` (§11, пункт 14). Отдельный `XFR-D-064 v1.0` сохраняет dataset-coverage owner/approver, missing/unclassified handling и non-compensation boundary при открытых segment universe/classification/numeric coverage; re-identification threshold остаётся отдельно `OPEN_BLOCKED_PENDING_DECISION` (§11, пункт 9).
...
**Replay — current `XFR-D-M4 v1.0` overlay.** Exact replay обязателен для deterministic path — идентичные input hashes, component scores, ranking, reasons и final package hash; ни bounded tolerance, ни rounding, ни aggregate compensation к deterministic mismatch не применимы, а mismatch остаётся severity-1 defect и блокирует affected rule version (Architecture §49, `SOURCE_NORMATIVE`). Recorded replay по сохранённому неперсональному response artifact и bounded replay внешнего вероятностного компонента остаются distinct; probabilistic output advisory only и не проходит Qualification Gate самостоятельно. Bounded mode требует одной separately approved closed immutable version/hash-bound candidate specification и evidence package с preregistration, frozen allocation, tuning/final isolation, compatible comparison и separate non-compensating reporting. Missing/stale/conflicting/incompatible input/artifact/provenance/version/evidence fails closed для affected replay/evidence progression без invented tolerance/pass/negative fact/route; replay создаёт новый audit event и не изменяет historical Match Result. Every exact tolerance/invariant/data/statistical/policy/production/carrier/runtime content remains `OPEN_BLOCKED_PENDING_DECISION`; successful replay не вызывает automatic policy/model/runtime/gate action.

---

## 9. Threshold-search boundary

Только qualitative procedure/evidence boundary, ни одного численного threshold или exact statistical method. `PARTIALLY_RESOLVED_BOUNDARY` по `XFR-D-070 v1.0`: governance owner — `AI + DEVELOPMENT`; mandatory approvers — `Chief AI Architect + PRODUCT + LEGAL`; evidence-procedure owner — `AI + DEVELOPMENT`, без unilateral approval authority. Это согласовано с Architecture §30.3 (review Chief AI Architect + согласование PRODUCT/LEGAL перед любым platform-level release) и §52 (manifest требует approval date/approver IDs):
...
`XFR-D-070 v1.0` дополнительно требует до доступа к untouched final outcomes зафиксировать versioned pre-registration evidence categories: claim/policy bundle, eligible population/unit/component universe/splits, proposed metric/denominator/aggregation/missing treatment, candidate search space/reference, planned comparisons/segments, proposed uncertainty/statistical/multiplicity/stopping/deviation treatment, interpretation/limitations и immutable data/code/configuration lineage. Это categories обязательного evidence, не approval их exact contents.

Post-selection, cherry-picking, selective reporting, unregistered repeated looks, result-dependent stopping/reruns и изменение metric/denominator/segments/method после просмотра final outcomes запрещены. Direct comparison требует совместимых evaluation units, eligibility/grouping/correction policies, schema/policy/metric versions and lineage; incompatible runs не получают delta/winner/pass claim. Statistical signal не становится automatic practical/legal/fairness/causal/production/release/gate verdict; absence of detected difference не доказывает equivalence/no harm/fairness/no drift; aggregate result не компенсирует adverse/unevaluable/insufficient metric/segment/intersection evidence. Synthetic-only evidence не создаёт production statistical claim.

Все exact hypotheses, paradigms, tests/estimators/intervals, significance/confidence/power/effect-size/sample-size values, multiple-comparison and sequential/stopping methods, equivalence/non-inferiority margins, aggregation rules, search algorithm and decision mappings остаются `OPEN_BLOCKED_PENDING_DECISION` (§11, пункт 16). Conventional/library defaults, pilot cap `100 Campaign` и Campaign→Qualified `40%/25%` не являются surrogate.

Для future Qualification-threshold evidence `XFR-D-045 v1.0` дополнительно требует baseline до candidate search, immutable frozen manifest, строгую component-level tuning/final isolation, separate full reporting/uncertainty/counter-evidence для mutual-fit, Confidence cutoff и completeness, applicable segment/intersection sufficiency и non-compensation. Это только mandatory categories/prerequisites: exact contents, procedure, numbers, dataset и evidence не утверждены. Evidence result не меняет автоматически threshold, ranking, Hard Constraint, routing, policy, model, release, gate или runtime.
...
Отдельно для `XFR-D-045` missing/absent/stale/incompatible/ambiguous/conflicting/leakage-contaminated/improperly post-freeze-corrected/incomplete/unauthorized evidence, manifest, baseline или final split блокирует только затронутую Qualification-threshold evidence progression fail closed. Он не считается zero/clean/pass/sufficient и не создаёт новую Qualification route, adverse fact, unrelated processing block или automatic `EVALUATION_RUN_REJECTED` mapping.
...
`XFR-D-071 v1.0` добавляет к этому candidate lifecycle только qualitative governance boundary: accepted immutable correction после `FROZEN` запускает source-authoritative impacted-run discovery; historical manifest/results/reviews/stage/hashes не меняются; impact evidence append-only; affected impact требует нового versioned cycle, unresolved impact блокирует reuse, а `demonstrably unaffected` требует positive complete proof. Это не новый lifecycle stage или runtime enum и не автоматическое назначение `EVALUATION_RUN_REJECTED`; exact notification/disposition schema, per-run quorum/RBAC/SLA, lifecycle/verdict mapping, remediation/rollback и implementation остаются `OPEN_BLOCKED_PENDING_DECISION`.
...
| 5 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-061 v1.0`.** Governance owner/approver и evidence-prerequisite boundary будущего false-exclusion maximum разрешены; maximum применяется только к confirmed successful pair, ошибочно исключённой Hard Filter на eligible verified labeling. Numeric maximum, baseline, exact numerator/denominator, aggregation window, uncertainty/confidence method и statistical test остаются `OPEN`; два отдельных Architecture §34.1 правила `0%` не получают tolerance/waiver/aggregation/compensation; Campaign→Qualified `40%/25%` не являются surrogate; `XFR-D-057`–`XFR-D-060` обязательны, qualitative `XFR-D-062 v1.0` и `XFR-D-070 v1.0` boundaries применяются, но numeric size/ratios/exact algorithm/seed policy/value `XFR-D-062`, contents `XFR-D-064`/`XFR-D-068` и exact/numeric statistical contents `XFR-D-070` остаются open где применимы; `XFR-D-063 v1.0` независимо разрешает только metric-target governance/evidence boundary и не подменяет maximum | `Chief AI Architect + AI` (governance owner `XFR-D-061`; mandatory approvers `PRODUCT + LEGAL + DEVELOPMENT`; evidence-procedure owner `AI + DEVELOPMENT`, не unilateral approver) | Owner/evidence boundary больше не blocking; numeric maximum, exact metric/statistics, complete evidence package и production prerequisites остаются blocking |
| 6 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-062 v1.0`.** Governance owner/approvers, component-atomic allocation, pre-freeze algorithm/input/component-universe/seed/assignment evidence, no-reroll/no-reseed/no-cherry-pick и fail-closed boundary разрешены. Assignment не создаёт label eligibility; `XFR-D-057`–`XFR-D-060` обязательны; pilot cap `100 Campaign` и conventional ratios не являются defaults. Numeric size/minimum/ratios/tolerance/counting denominator, exact allocation/stratification algorithm, seed-generation policy/value и actual dataset/manifest/run остаются `OPEN`; numeric `XFR-D-061`, numeric targets/statistics `XFR-D-063` и `XFR-D-064`/`068`/`070`/`071` не подменяются | `AI + DEVELOPMENT` (governance owner `XFR-D-062`; mandatory approvers `Chief AI Architect + PRODUCT + LEGAL`; evidence-procedure owner `AI + DEVELOPMENT`, не unilateral approver) | Qualitative allocation/reproducibility boundary больше не blocking; numeric sufficiency/allocation policy, complete manifest, data/privacy и runtime evidence остаются blocking |
| 7 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-063 v1.0`.** Governance owner будущих in-scope targets `Precision@K`/`Recall@K`/`NDCG@K`/Confidence Score calibration/upper-result diversification — `Chief AI Architect + AI`; mandatory approvers — `PRODUCT + LEGAL + DEVELOPMENT`; evidence-procedure owner — `AI + DEVELOPMENT`, без unilateral approval. `XFR-D-057`–`XFR-D-062` обязательны; baseline-first, tuning/final separation и no-cross-metric-compensation сохраняются. Все numeric targets, `K`, exact metric/relevance definitions, numerator/denominator/aggregation, calibration/diversification methods, uncertainty/confidence и statistical comparison остаются `OPEN`; Risk Score/human-review, `XFR-D-061`, rank/replay, critical-risk/Qualification, `XFR-D-064`/`068`/`070` и runtime не подменяются | `Chief AI Architect + AI` (governance owner `XFR-D-063`; mandatory approvers `PRODUCT + LEGAL + DEVELOPMENT`; evidence-procedure owner `AI + DEVELOPMENT`, не unilateral approver) | Owner/evidence boundary больше не blocking; numeric targets, exact metric/statistics, complete evidence package и Model release/Launch остаются blocking |
| 8 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-064 v1.0`.** Governance owner/approver, evidence-procedure role, missing/unclassified-segment fail-closed handling (не negative/failed/majority, не AI/heuristic/proxy-imputed, не молча исключается, отдельный diagnostic-only bucket) и non-compensation boundary (aggregate success не компенсирует segment/intersection insufficiency) разрешены. Privacy small-cell suppression (Architecture §8.4) остаётся отдельно от statistical small-cell sufficiency. Segment universe, protected/proxy классификация и lawful-basis determination (`LeaseMind_MATCHING_RISK_POLICY_v0.1.md` §13 открытое решение №9, `LEGAL + PRODUCT`; Feature Schema №9/№17), intersection definitions, numerator/denominator/counting unit и любое численное minimum/ratio/threshold/tolerance/aggregation/statistical procedure остаются `OPEN`; `XFR-D-068`/`XFR-D-M3`/`XFR-D-063`/`XFR-D-061`/`XFR-D-062`/Scoring segment-override(`XFR-D-018`)/ranking algorithm(`XFR-D-021`) не подменяются | `PRODUCT + LEGAL` (governance owner `XFR-D-064`; mandatory approvers `Chief AI Architect + AI + DEVELOPMENT`; evidence-procedure owner `AI + DEVELOPMENT`, не unilateral approver) | Governance owner/evidence-prerequisite boundary больше не blocking; segment universe, protected/proxy classification, numeric coverage minimum, complete evidence package и production prerequisites остаются blocking |
| 9 | **`PARTIALLY_RESOLVED_BOUNDARY` — merged `XFR-D-M3 v1.0` (`EP-09`, `PRIMARY_MERGED_MEMBER`).** Только role split, four-context separation, closed immutable version/hash-bound candidate discipline, affected-use fail-closed, non-compensation, source-normative deny preservation, historical binding, no-automatic-action и qualitative evidence prerequisites утверждены; exact method/cohort/quasi-ID/adversary/linkage/numerator/denominator/counting/threshold/comparator/tolerance/statistics/data/evidence/policy/production/schema/runtime/implementation остаются `OPEN` | `PRODUCT + LEGAL` — human-approved candidate-derived governance owner, не `SOURCE_NORMATIVE` для whole merge; Evaluation artifact owner `AI + DEVELOPMENT`; mandatory approvers `Chief AI Architect + AI + DEVELOPMENT`; evidence/technical-procedure `AI + DEVELOPMENT` без unilateral authority | Exact re-identification method/threshold, de-identification/evaluation evidence and production applicability |
| 10 | **`PARTIALLY_RESOLVED_BOUNDARY` — merged `XFR-D-M4 v1.0` (`EP-10`, `PRIMARY_MERGED_MEMBER`).** Approved only: deterministic exact/no-tolerance/severity-1 preservation; recorded-versus-bounded probabilistic replay separation; advisory-only probabilistic use; closed immutable version/hash-bound candidate/evidence discipline; preregistration, frozen allocation, tuning/final isolation, compatible comparison, separate reporting; affected-use fail-closed; non-compensation/no masking; historical Match Result immutability; no automatic action | `DEVELOPMENT + AI` — human-approved candidate-derived governance owner, not `SOURCE_NORMATIVE`; mandatory approvers `Chief AI Architect + PRODUCT + LEGAL`; evidence/technical owner `DEVELOPMENT + AI` without unilateral authority; Evaluation/Risk/Qualification/Scoring artifact owners remain separate | Every exact tolerance value/range/unit/error metric/direction, absolute/relative/per-component/aggregate treatment, reason-code invariant catalog, probabilistic eligibility, recorded-artifact schema/retention/provenance, dataset/splits/sample/metrics/targets/statistics, representation/precision/rounding, policy/manifest approval, production applicability, API/DB/events/carrier/runtime/monitoring/rollback/implementation remains `OPEN`; exact contents still block replay/determinism acceptance |
| 11 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-065 v1.0`.** Governance owner `AI + DEVELOPMENT`; mandatory approvers `Chief AI Architect + PRODUCT + LEGAL`; monitoring/evidence-procedure owner `AI + DEVELOPMENT`, без unilateral approval. Architecture §30.3 п.9/§54 задают только monitoring/rollback capability и отдельный versioned operational artifact с owner/runbook/escalation для каждого alert, но не drift semantics. Evaluation Plan остаётся evidence artifact, а production monitoring/SLO — `OUT_OF_SCOPE`. Missing/stale/incompatible/lineage-unverified telemetry не является no-drift/pass evidence; monitoring failure и detected drift различаются; signal/alert не выполняет automatic model/policy/routing/release/rollback action; aggregate не маскирует insufficient evidence; synthetic-only evidence не создаёт production claim. Drift taxonomy, monitored objects/populations, baseline/reference set, metrics, numerator/denominator, windows/sample size, thresholds/tolerances, statistics, aggregation/weighting, alert levels/SLO, named appointments/RBAC, runbooks/escalation, action/rollback triggers, operational artifact, runtime и implementation остаются `OPEN` | `AI + DEVELOPMENT` (governance owner `XFR-D-065`; mandatory approvers `Chief AI Architect + PRODUCT + LEGAL`; monitoring/evidence-procedure owner `AI + DEVELOPMENT`, не unilateral approver) | Owner/artifact/evidence boundary больше не blocking; exact drift policy, complete evidence package, production operational-artifact approval и runtime/implementation остаются blocking |
| 12 | **`RESOLVED_PROCEDURAL_GOVERNANCE_BOUNDARY` — `XFR-D-066 v1.0`.** Artifact owner остаётся `AI + DEVELOPMENT` по Architecture §52; approval-flow governance owner — `Chief AI Architect + AI + DEVELOPMENT`; Chief AI Architect — mandatory independent architecture reviewer/coordinator, не unilateral approver; `PRODUCT + LEGAL` — mandatory domain approvers текущего v0.1. Все required roles подписывают один frozen semantic version/SHA-256; sequence: owner readiness → Chief review → PRODUCT/LEGAL determinations → same-hash finalization → separate actual approval record → Controlled Artifact Manifest entry last. Для future version `NOT_APPLICABLE` может подписать только сама PRODUCT/LEGAL role на том же hash с scope/rationale; ambiguity блокирует flow fail closed, новый runtime/manifest enum не вводится. Любое изменение bytes/hash обнуляет текущую попытку approval. Current Plan остаётся Proposal; actual approval, manifest entry, substantive dependencies, named appointments/RBAC/signature carrier/schema, dataset и runtime остаются `OPEN` | `Chief AI Architect + AI + DEVELOPMENT` (approval-flow governance owner; artifact owner `AI + DEVELOPMENT`; Chief AI Architect independent reviewer/coordinator; current v0.1 domain approvers `PRODUCT + LEGAL`) | Процедура больше не blocking; её фактическое выполнение на одном frozen hash, отдельный approval record, Controlled Artifact Manifest entry и все substantive/data/runtime prerequisites остаются blocking |
...
| 14 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-068 v1.0`.** Governance owner `LEGAL + PRODUCT`; mandatory approvers `Chief AI Architect + AI + DEVELOPMENT`; evidence-procedure owner `AI + DEVELOPMENT`, без unilateral legal/product approval. Architecture §30.3 п.4 требует discrimination/proxy check, но не задаёт exact framework/standard. Diagnostic disparity — signal, не automatic discrimination/lawfulness/causation verdict; отсутствие detected disparity или protected/segment labels не доказывает fairness/readiness; aggregate не компенсирует adverse/unevaluable/insufficient segment/intersection evidence. Architecture §17 Risk Score protected/proxy ban не получает waiver; diagnostic-only sensitive-dimension use требует отдельной purpose-specific LEGAL/PRODUCT/Data Governance/privacy authority и не переносится в model/scoring/routing/runtime. Exact fairness doctrine, protected/proxy classification/lawful basis, population/segments/intersections, outcome/comparator, metric/numerator/denominator, numeric threshold/tolerance, aggregation/weighting, uncertainty/statistics/causal method/remediation, dataset/run и production applicability остаются `OPEN` | `LEGAL + PRODUCT` (governance owner `XFR-D-068`; mandatory approvers `Chief AI Architect + AI + DEVELOPMENT`; evidence-procedure owner `AI + DEVELOPMENT`, не unilateral approver) | Governance/non-conflation/evidence boundary больше не blocking; exact legal standard, classification, metric/statistics, evidence package, data authority и production/runtime prerequisites остаются blocking |
...
| 16 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-070 v1.0`.** Governance owner `AI + DEVELOPMENT`; mandatory approvers `Chief AI Architect + PRODUCT + LEGAL`; evidence-procedure owner `AI + DEVELOPMENT`, без unilateral approval. Pre-registration evidence categories до доступа к final outcomes, tuning/final component isolation, compatible-version/run comparison, full reporting of adverse/null/incompatible/unevaluable/insufficient outcomes and deviations, non-compensation и no-automatic-verdict/action boundaries разрешены. Post-selection, cherry-picking, selective reporting, unregistered repeated looks and result-dependent stopping/reruns запрещены. Все exact hypotheses/paradigms/tests/estimators/intervals, significance/confidence/power/effect-size/sample-size values, multiple-comparison/sequential/stopping/equivalence methods, aggregation, threshold-search algorithm and decision mappings остаются `OPEN`; `XFR-D-057`–`XFR-D-069` не подменяются, а qualitative `XFR-D-071 v1.0` boundary не заменяет exact corrected-data treatment/statistics | `AI + DEVELOPMENT` (governance owner `XFR-D-070`; mandatory approvers `Chief AI Architect + PRODUCT + LEGAL`; evidence-procedure owner `AI + DEVELOPMENT`, не unilateral approver) | Governance/pre-registration/tuning-final/compatibility/reporting boundary больше не blocking; exact/numeric statistics, complete evidence package, policy approval and production/runtime prerequisites остаются blocking |
| 17 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-071 v1.0`.** Governance owner `AI + DEVELOPMENT`; mandatory approvers `Chief AI Architect + PRODUCT + LEGAL`; evidence-procedure owner `AI + DEVELOPMENT`, без unilateral approval и без автоматического назначения per-run quorum. Только accepted immutable correction record запускает boundary; rejected/no-op/replay не запускают. Source-authoritative lineage определяет impacted runs; missing/ambiguous evidence fails closed. Historical manifests/results/reviews/stages/hashes immutable, impact evidence append-only; affected impact требует new versioned cycle, unresolved остаётся blocked, а `demonstrably unaffected` требует positive proof. Governance meanings не runtime enums; exact notification/disposition schema, query/detector, quorum/RBAC/SLA, lifecycle/verdict mapping, remediation/rollback и runtime implementation остаются `OPEN` | `AI + DEVELOPMENT` (governance owner; mandatory approvers `Chief AI Architect + PRODUCT + LEGAL`; evidence-procedure owner `AI + DEVELOPMENT`, не unilateral approver и не per-run quorum) | Qualitative trigger/discovery/immutability/no-silent-reuse boundary больше не blocking; exact operational procedure, actual impact evidence/run, production-data and runtime prerequisites remain blocking |

Решение №1 получило qualitative eligibility boundary `XFR-D-057`; решение №2 — human adjudication governance procedure `XFR-D-058 v1.1`; решение №3 — grouping/isolation boundary `XFR-D-059 v1.1`; решение №4 — conservative correction-history exclusion boundary `XFR-D-060 v1.0`; решение №5 — только governance owner/approver и evidence-prerequisite boundary `XFR-D-061 v1.0`, при полностью открытом numeric maximum и exact metric/statistics; решение №6 — только governance owner/approver, component-atomic allocation, pre-freeze/no-reroll и fail-closed boundary `XFR-D-062 v1.0`, при открытых numeric size/ratios/tolerance/exact algorithm/seed policy/value и actual dataset/run; решение №7 — только governance owner/approver, metric-family separation, baseline-first/tuning-final и evidence-prerequisite boundary `XFR-D-063 v1.0`, при полностью открытых numeric targets/`K`/exact metric-statistics; решение №8 — только governance owner/approver, evidence-procedure role, missing/unclassified-segment fail-closed handling и non-compensation boundary `XFR-D-064 v1.0`, при полностью открытых segment universe, protected/proxy классификации и любом численном coverage minimum; решение №11 — только governance owner/approver, separation Evaluation Plan от production operational artifact, fail-closed monitoring-evidence handling, failure-vs-drift separation, non-compensation и no-automatic-action boundary `XFR-D-065 v1.0`, при полностью открытых drift taxonomy/metrics/baselines/windows/triggers/statistics/alerts/actions/operational artifact/runtime; решение №12 получило только cross-functional approval procedure boundary `XFR-D-066 v1.0`, тогда как current Plan остаётся Proposal, actual same-hash approval record и Controlled Artifact Manifest entry отсутствуют; решение №13 разрешено отдельным human governance record; решение №14 получило только governance owner/approver, diagnostic-vs-legal non-conflation, protected/proxy diagnostic-use и evidence-prerequisite boundary `XFR-D-068 v1.0`, при полностью открытых exact fairness standard/classification/metrics/thresholds/statistics/remediation; строка №15 получила qualitative terminology boundary `XFR-D-069`; решение №16 получило только governance owner/approver, pre-registration, tuning/final isolation, comparison-compatibility, full-reporting/non-compensation и no-automatic-verdict/action boundary `XFR-D-070 v1.0`, при полностью открытых exact/numeric statistics. Эти records не одобряют dataset или Evaluation Plan; actual approval execution/evidence, named appointments/RBAC, numeric части `XFR-D-061`/`XFR-D-062`/`XFR-D-063`, segment universe/protected-classification/numeric части `XFR-D-064`, exact/operational часть `XFR-D-065`, exact/content часть `XFR-D-068`, exact/numeric statistical contents `XFR-D-070`, source-history/runtime representation, `XFR-D-071` post-freeze synchronization, production-data prerequisites и прочие remaining dependencies остаются `OPEN`, а новые gaps добавляются будущими ревью, не разрешаются этим документом.

---

**Current row №17 overlay:** `XFR-D-071 v1.0` разрешает только governance owner/approvers/evidence role, accepted-correction trigger, source-authoritative discovery, immutable-history/append-only evidence and no-silent-reuse boundary. Любое прежнее упоминание `XFR-D-071` как целиком `OPEN` относится к exact notification/disposition/runtime contents; сам qualitative boundary имеет `PARTIALLY_RESOLVED_BOUNDARY`.

**Current external Qualification row №19 overlay — register остаётся ровно 17 строк:** `MQP-19 → XFR-D-045`, `PRIMARY_STANDALONE`, имеет `PARTIALLY_RESOLVED_BOUNDARY` только для decision-specific owner/approvers/evidence role, `XFR-F1` recognition, three-family separation/non-compensation, baseline-first/frozen-manifest/tuning-final and prerequisite-only/fail-closed discipline. Этот overlay не добавляет строку в Evaluation Plan register и не создаёт Qualification-specific metric family; actual `XFR-F1` content, procedure, dataset/run/evidence/results/verdict, production applicability, policy/manifest, runtime и implementation остаются `OPEN`.

> **Temporal hygiene for row №9:** historical wording that calls the re-identification method/threshold wholly `OPEN` is superseded by the current `XFR-D-M3` overlay: only the qualitative boundary is `PARTIALLY_RESOLVED_BOUNDARY`; every exact method, threshold, data, evidence, policy, production and runtime content remains `OPEN`.
...
**Then:** финальная проверка выполняется на данных и connected components, не использованных для поиска этого же threshold; component-level overlap отклоняется по `MEP-C-002`. Для `XFR-D-045` baseline измеряется до candidate search, manifest frozen, а tuning evidence никогда не переиспользуется как final evidence.
...
**Then:** изменение не выполняется автоматически; требуется процедура §9 и Architecture §30.3 (review, согласование, контролируемый выпуск). `XFR-D-045` также запрещает automatic threshold, ranking, Hard Constraint, routing, policy, model, release, gate или runtime change.
...
**Then:** соответствующая candidate version правил блокируется (severity-1, Architecture §49); run получает evidence verdict `EVALUATION_RUN_REJECTED` (§10) и не засчитывается как пройденный. Merged `XFR-D-M4 v1.0` сохраняет deterministic replay exact-only и не разрешает tolerance, rounding или aggregate compensation. Recorded и bounded probabilistic replay остаются distinct; bounded mode требует separately approved closed immutable version/hash-bound candidate and frozen/preregistered evidence, а every exact tolerance/invariant/data/statistical/policy/production/carrier/runtime content remains `OPEN`.
...
**Then:** ни один статус не меняется автоматически; это отдельные cross-functional решения (§10, пункт 3–4). `XFR-D-M4 v1.0` дополнительно запрещает automatic policy/model/runtime/gate action по результату replay, сохраняет probabilistic output advisory-only и historical Match Result immutable; all three gates remain `BLOCKED`.

---

## 13. Readiness/DoD

`XFR-D-045 v1.0` не создаёт `XFR-F1` metric-family content, не одобряет Qualification threshold, dataset/manifest/run/evidence package/result/verdict, production applicability, Evaluation Plan или runtime. Его external overlay не меняет 17-row register.

`XFR-D-071 v1.0` не одобряет этот Evaluation Plan, dataset/manifest/run, actual impact package, production-data use, exact notification/disposition procedure, policy/model release, runtime или implementation. Row №17 остаётся `PARTIALLY_RESOLVED_BOUNDARY`; exact operational contents остаются blocking.

`XFR-D-M4 v1.0` не одобряет tolerance, reason-code invariant catalog, dataset/run/result, Evaluation/Risk/Qualification/Scoring Policy, manifest, production applicability, carrier/runtime или implementation. Row №10 остаётся одной из 17 register rows и получает только `PARTIALLY_RESOLVED_BOUNDARY`; its exact contents remain blocking. `XFR-D-020`/`023`/`026`/`027`, `XFR-D-057`–`071` and all sibling Evaluation/Risk/Qualification/Scoring boundaries remain independent.

`XFR-D-M3 v1.0` не одобряет re-identification method/threshold, dataset, diagnostic result, policy, production applicability, schema/carrier/runtime или implementation. Row №9 остаётся одной из 17 register rows и получает только `PARTIALLY_RESOLVED_BOUNDARY`; exact method/cohort/quasi-ID/adversary/linkage/numerator/denominator/counting/threshold/comparator/tolerance/statistics/data/evidence remain blocking. Four source keys remain `PRIMARY_MERGED_MEMBER`; all three gates remain `BLOCKED`.

Настоящий документ:

- готов только к cross-functional review — не более;
- не закрывает вопрос №10 Architecture §37 — вопрос остаётся `OPEN`;
- сохраняет merged `EP-10 + MRP-11 + MQP-15 + MSP-14 → XFR-D-M4` с четырьмя `PRIMARY_MERGED_MEMBER`; синхронизирует только approved qualitative replay governance/evidence safeguards, не меняя 17-row register;
- decision register сохраняет 17 строк: №1 получил qualitative eligibility boundary `XFR-D-057`, №2 — human adjudication governance procedure `XFR-D-058 v1.1`, №3 — grouping/isolation boundary `XFR-D-059 v1.1`, №4 — conservative correction-history exclusion boundary `XFR-D-060 v1.0`, №5 — только governance owner/approver и evidence-prerequisite boundary `XFR-D-061 v1.0` (`PARTIALLY_RESOLVED_BOUNDARY`), №6 — только governance owner/approver, component-atomic allocation, pre-freeze/no-reroll и fail-closed boundary `XFR-D-062 v1.0` (`PARTIALLY_RESOLVED_BOUNDARY`), №7 — только governance owner/approver, metric-family separation, baseline-first/tuning-final и evidence-prerequisite boundary `XFR-D-063 v1.0` (`PARTIALLY_RESOLVED_BOUNDARY`), №8 — только governance owner/approver, evidence-procedure role, missing/unclassified-segment fail-closed handling и non-compensation boundary `XFR-D-064 v1.0` (`PARTIALLY_RESOLVED_BOUNDARY`), №11 — только governance owner/approver, artifact-separation, fail-closed monitoring-evidence handling, failure-vs-drift separation, non-compensation и no-automatic-action boundary `XFR-D-065 v1.0` (`PARTIALLY_RESOLVED_BOUNDARY`), №12 — только approval procedure boundary `XFR-D-066 v1.0` (`RESOLVED_PROCEDURAL_GOVERNANCE_BOUNDARY`; actual approval/manifest entry pending), №14 — только governance owner/approver, diagnostic-vs-legal non-conflation, protected/proxy diagnostic-use and evidence-prerequisite boundary `XFR-D-068 v1.0` (`PARTIALLY_RESOLVED_BOUNDARY`), №15 — qualitative terminology boundary `XFR-D-069`, №16 — только governance owner/approver, pre-registration, tuning/final isolation, comparison compatibility, full-reporting/non-compensation и no-automatic-verdict/action boundary `XFR-D-070 v1.0` (`PARTIALLY_RESOLVED_BOUNDARY`); numeric false-exclusion maximum/baseline/exact metric-statistics, numeric dataset size/minimum/ratios/tolerance/counting denominator, exact allocation/stratification algorithm, seed policy/value, actual dataset/manifest/run, actual Plan approval/approval record/Controlled Artifact Manifest entry, named appointments/RBAC/signature carrier, все numeric targets/`K`/exact metric-statistics `XFR-D-063`, segment universe/protected-proxy classification/lawful-basis determination/numeric coverage minimum `XFR-D-064`, drift taxonomy/monitored population/baseline/metrics/windows/triggers/statistics/alerts/actions/operational artifact/runtime `XFR-D-065`, exact fairness standard/doctrine/classification/population/comparators/metrics/thresholds/statistics/causal/remediation contents `XFR-D-068`, exact/numeric statistical contents `XFR-D-070`, `XFR-D-071` post-freeze synchronization, source-history/runtime controls, прочие численные targets, re-identification threshold, runtime/reporting mappings и остальные dependencies остаются открытыми (§11);
- сохраняет external `MQP-19 → XFR-D-045` как `PARTIALLY_RESOLVED_BOUNDARY` без новой Evaluation register row или metric family; `XFR-F1`, exact/numeric/statistical content, dataset/run/evidence, production applicability, policy/manifest/runtime/implementation остаются `OPEN`;
