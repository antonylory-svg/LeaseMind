...
- **Owner финального threshold:** legal fairness standard — `LEGAL + PRODUCT`; процедура diagnostic — `AI`; оба — `DECISION_CANDIDATE_FOR_REVIEW`, поддержанные §30.3 (координация PRODUCT/LEGAL при platform-level release), но не названные источником owner'ами напрямую; см. открытое решение №14. Governance owner будущей **dataset-coverage sufficiency** policy (отдельный вопрос от legal fairness standard) утверждён `LeaseMind_MATCHING_DECISION_XFR-D-064_v1.0.md`: `PRODUCT + LEGAL`; mandatory approvers `Chief AI Architect + AI + DEVELOPMENT`; evidence-procedure owner `AI + DEVELOPMENT`, не unilateral approver.
- **Статус:** `SOURCE_NORMATIVE` для требования самой проверки — «проверку дискриминационных признаков и прокси» (Architecture §30.3, пункт 4). `PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-064 v1.0`: governance owner/approvers, evidence-procedure role, missing/unclassified-segment fail-closed handling и non-compensation boundary для segment-coverage diagnostics утверждены; segment universe, protected/proxy классификация, intersections, numerator/denominator и любое численное coverage minimum остаются `OPEN_BLOCKED_PENDING_DECISION` (§11, пункт 8). Re-identification threshold остаётся отдельно `OPEN_BLOCKED_PENDING_DECISION` (§11, пункт 9) и не подменяется `XFR-D-064`.
