...
Документ предлагает процедурный каркас для review вопроса №10 Architecture §37 («Какая размеченная выборка и процедура adjudication используются для pilot baseline?», owner `AI + DEVELOPMENT`, `SOURCE_NORMATIVE` для формулировки вопроса и его owner), но не закрывает ни часть, ни весь вопрос №10 до утверждения владельцами. Сам процедурный каркас — `DECISION_CANDIDATE_FOR_REVIEW`: не назначает конкретную выборку и не проводит adjudication.
...
### 4.1. Кандидаты unit of evaluation

`Property`, `TenantRequest`, Campaign, `match_pair_id`, `encounter_id`, `(aggregate_id, source_revision)` — все перечислены только как кандидаты granularity. `DECISION_CANDIDATE_FOR_REVIEW` для самого перечня кандидатов (основание — существование этих ID в `CAMPAIGN_TECHNICAL_ASSIGNMENT.md` §13 и Architecture §21.3/§48.8). Выбор конкретного обязательного уровня группировки — `OPEN_BLOCKED_PENDING_DECISION` (см. §11, пункт 3).
...
- approved grouping/isolation policy version и конкретные split assignments/hashes (§4.2, §11 пункт 3);
...
- отсутствии approved grouping/isolation policy, либо incomplete freeze-time manifest (§8.1) — блокирует переход в `FROZEN`;
...
| --- | --- | --- | --- |
| 1 | **`RESOLVED_QUALITATIVE_ELIGIBILITY_BOUNDARY` — `XFR-D-057 v1.0`.** Five-category eligibility matrix утверждена: deterministic fixtures вне feedback enum; expert/gate/safety/business eligibility условна и требует applicable approved source/reviewer procedure; `SELF_REPORTED` preference — только diagnostic/user-specific; unknown combinations fail closed; exact reviewer/adjudication workflow, quorum, runtime representation и production-data use остаются `OPEN` | `AI + DEVELOPMENT + LEGAL` (governance owner `XFR-D-057`; mandatory approvers `Chief AI Architect + PRODUCT`) | Qualitative ground-truth eligibility больше не blocking; exact label/adjudication contract и production-data prerequisites остаются blocking |
| 2 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-058 v1.0`.** Fail-closed ground-truth eligibility для `DISPUTED`/`INCONCLUSIVE` утверждена; qualitative evidence-level mapping разрешена `XFR-D-057`; exact adjudication workflow, blind/double review и quorum остаются `OPEN` | `AI + LEGAL` (governance owner `XFR-D-058`; mandatory approvers `Chief AI Architect + PRODUCT + DEVELOPMENT`) | Exact label adjudication contract остаётся blocking; silent coercion и qualitative mapping boundaries больше не blocking |
...
Решение №1 получило qualitative eligibility boundary `XFR-D-057`; решение №13 разрешено отдельным human governance record; строки №2/№3 частично резолвлены `XFR-D-058`/`XFR-D-059`, строка №15 получила qualitative terminology boundary `XFR-D-069`. Эти records не одобряют dataset или Evaluation Plan; exact reviewer/adjudication workflow, grouping policy, runtime representation, production-data prerequisites и прочие remaining dependencies остаются `OPEN`, а новые gaps добавляются будущими ревью, не разрешаются этим документом.
...
**Then:** финальная проверка выполняется на данных, не использованных для поиска этого же threshold.

#### `MEP-C-012` — freeze-time manifest обязателен до `FROZEN`

**Given:** любой evaluation run.
**When:** run переходит в `FROZEN`.
**Then:** freeze-time manifest (§8.1) содержит полный состав, включая approved grouping/isolation policy version и split assignments/hashes; run без полного freeze-time manifest не переходит в `FROZEN` и получает evidence verdict `EVALUATION_RUN_REJECTED` (§10).
...
- decision register сохраняет 17 строк: №1 получил qualitative eligibility boundary `XFR-D-057`, №2/№3 — только partial fail-closed boundaries `XFR-D-058`/`XFR-D-059`, №15 — qualitative terminology boundary `XFR-D-069`; exact reviewer/adjudication procedure, grouping/split policy, численные targets, drift monitoring, fairness standard, re-identification threshold, runtime/reporting mappings и остальные dependencies остаются открытыми (§11);
