...
| `BLOCKED_PENDING_DECISION` | Открытые решения №1, 4–6, 8–10, 17–19 §10 целиком; row №7 / `XFR-D-M3` имеет только `PARTIALLY_RESOLVED_BOUNDARY`, при этом exact method/cohort/quasi-ID/adversary/linkage/numerator/denominator/counting/threshold/comparator/tolerance/statistics/data/evidence/policy/production/schema/carrier/runtime/implementation остаются blocked; №11 — `RESOLVED_V0_1_SCOPE_BOUNDARY` `XFR-D-009 v1.0`; №14 — `RESOLVED_QUALITATIVE_LITERAL_BASELINE` `XFR-D-011 v1.0`; exact/content portion №12 after merged `XFR-D-M6 v1.0` и №13 after `XFR-D-010 v1.0` остаются blocked при разрешённом только `PARTIALLY_RESOLVED_BOUNDARY`; remaining unresolved portion №2/№3/№15 after Wave 2B — 6 cells `entrance_requirement_fit`, 13 cells `access_mode_hard_fit`, numeric-to-category convention and exact derived wildcard `value_state` `floor_option_fit`, plus `security_deposit_fact`, `budget_headroom` direction, `required_evidence_level` for all 20 candidates and final LEGAL verdict |
