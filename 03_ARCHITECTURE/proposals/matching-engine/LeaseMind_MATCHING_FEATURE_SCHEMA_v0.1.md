...
**Связанные документы:** `LeaseMind_MATCHING_ENGINE_ARCHITECTURE_v1.1.md`, `LeaseMind_MATCHING_DATA_CONTRACTS_v1.0.md` (только как существующая граница исполнимых контрактов, не источник feature-арифметики), `02_PRODUCT/CAMPAIGN_TECHNICAL_ASSIGNMENT.md`, `05_DEVELOPMENT/matching-engine/reviews/LeaseMind_DEVELOPMENT_REVIEW_MATCHING_ENGINE_v1.1_EIGHTH.md`, `LeaseMind_MATCHING_DECISION_XFR-D-001_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-002_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-012_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-013_v1.0.md` (Wave 2B human-approved governance decisions — status sources для §5.1/§5.2/§5.3/§10/§11, не approval этого Proposal), `05_DEVELOPMENT/matching-engine/decisions/LeaseMind_MATCHING_CROSS_FUNCTIONAL_DECISION_INVENTORY_v1.0.md` (governance status overlay, не approval)
...
Статус строк ниже не единообразно `open`: часть решений теперь `PARTIALLY RESOLVED` (human-approved qualitative governance decision резолвит часть, remaining часть открыта) или `RESOLVED_QUALITATIVE_BOUNDARY` (qualitative rule утверждена целиком, exact runtime representation остаётся отдельным open вопросом) благодаря Wave 2B decision records `XFR-D-001`/`XFR-D-002`/`XFR-D-012`/`XFR-D-013 v1.0`. Эти records — authority для соответствующей qualitative interpretation источника, не сам Feature Schema Proposal; ни одна строка не переводится в полностью закрытую этим документом.
...
**Given:** правило сравнения с `registry_readiness = BLOCKED_PENDING_DECISION` (например, remaining unresolved портion №8, 15, 20 после `XFR-D-001`/`XFR-D-002`/`XFR-D-012 v1.0` — partial-but-overall-blocked, с `readiness_reason = BLOCKED_PENDING_COMPATIBILITY_TABLE` там, где применимо; №17–19 — метод строкового сопоставления). §5.3 operating-expenses basis mismatch больше не пример этого случая ни в какой части: его qualitative comparison rule утверждена `XFR-D-013 v1.0` как `READY_AS_CANDIDATE_ONLY`, не `BLOCKED_PENDING_DECISION`; exact runtime representation этого правила остаётся open follow-up, полностью описанным `MFS-C-013`/§5.3, а не этим AC.
