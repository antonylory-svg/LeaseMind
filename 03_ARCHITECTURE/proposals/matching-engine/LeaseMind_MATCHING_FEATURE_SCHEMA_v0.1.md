...
**Current row №12 overlay:** human-approved merged `XFR-D-M6 v1.0` разрешает только `PARTIALLY_RESOLVED_BOUNDARY` для qualitative Feature Fit ↔ feature/value-level Evidence Confidence governance and evidence prerequisites. Governance owner `Chief AI Architect + AI` is candidate-derived and explicitly not `SOURCE_NORMATIVE`; Feature Schema artifact owner remains `PRODUCT + LEGAL + AI`, Scoring Policy artifact owner `Chief AI Architect + PRODUCT`, Evaluation Plan/evidence artifact owner `AI + DEVELOPMENT`; mandatory approvers are `PRODUCT + LEGAL + DEVELOPMENT`, and evidence/technical-procedure owner `AI + DEVELOPMENT` has no unilateral authority. Feature Fit, Feature Weight, categorical `evidence_status`, Evidence Confidence, `required_evidence_level`, overall Confidence, Hard Constraint/Eligibility, Risk and Qualification remain separate; exact seven Architecture §13 statuses, affected-use fail-closed, non-compensation/double-counting review, immutable evidence, historical binding and no-automatic-action safeguards are preserved. Every exact mapping/function/order/hierarchy/value/range/direction/default/fallback, including `[0,1]` as an approved contract, calibration/data/statistics/policy/production/schema/carrier/runtime/implementation content remains `OPEN`; all three gates remain `BLOCKED`.

**Current row №1 overlay:** human-approved merged `XFR-D-M1 v1.0` разрешает только `PARTIALLY_RESOLVED_BOUNDARY` для per-feature/source/use/purpose required-evidence governance and qualitative evidence prerequisites. Governance owner `PRODUCT + AI + Chief AI Architect` is human-approved candidate-derived and explicitly not `SOURCE_NORMATIVE`; Feature Schema artifact owner remains separately `PRODUCT + LEGAL + AI`; Qualification Policy artifact owner remains separately `Chief AI Architect + PRODUCT` under `XFR-D-030 v1.0`; mandatory approvers are `LEGAL + DEVELOPMENT`; evidence/technical-procedure owner `AI + DEVELOPMENT` has no unilateral authority. Exact seven-value Architecture §13 enum, validation-not-confirmation, semantic separation, affected-use-only fail-closed, non-compensation/no-double-counting, immutable evidence, historical binding and no-automatic-action safeguards are preserved. Every exact level, assignment, sufficiency mapping/order/hierarchy/default/fallback, freshness/conflict/expiry/revocation rule, reviewer workflow, Qualification consequence, data, Policy, production, schema/carrier/runtime and implementation content remains `OPEN`; `automatic_ineligible_allowed = NO` remains unchanged and all three gates remain `BLOCKED`.

**Current G1 legal-admissibility overlay:** human-approved `XFR-D-004`, `XFR-D-007`, `XFR-D-014` and `XFR-D-016 v1.0` update only their four qualitative governance boundaries and exact role splits. `FS-05` is `PARTIALLY_RESOLVED_BOUNDARY` for Identity/Authority Registry sole-source and future purpose/scope-compatible immutable versioned read-only `GATE_ONLY` consumption; `FS-09` is `RESOLVED_V0_1_SCOPE_BOUNDARY`, with `business_stage_signal` excluded from every v0.1 Matching use while raw `request_business_stage` remains unchanged; `FS-17` is `PARTIALLY_RESOLVED_BOUNDARY`, with no final LEGAL verdict for any of the 20 candidates, all remaining `ELIGIBILITY_HARD_CONSTRAINT_CANDIDATE` and `automatic_ineligible_allowed = NO`; `FS-19` is `PARTIALLY_RESOLVED_BOUNDARY` for Lawful Basis/Consent Registry sole-writer and future purpose-bound, scope-compatible, versioned, read-only projection/ref consumption with applicable source-owned invalidation. Exact carriers, mappings, TTL/freshness, evidence packages, classifications, verdicts, RBAC and operational consequences remain `OPEN`; no Proposal, Policy, Data Contract, dataset, evaluation, production, runtime or implementation is approved and all three gates remain `BLOCKED`.

**Current G2 feature-input-semantics overlay:** human-approved `XFR-D-003`, `XFR-D-006`, `XFR-D-008` and `XFR-D-015 v1.0` update only their four qualitative governance boundaries and exact role splits. `FS-04` is `PARTIALLY_RESOLVED_BOUNDARY` for a future explicit immutable version/hash-bound representation contract at both derived `effective_rate` calculation and comparison checkpoints, while every exact numeric detail remains `OPEN`; `FS-08` is `RESOLVED_QUALITATIVE_BOUNDARY`, with separately attributable Property/Request `deal_priority` values limited to a future separately governed ranking-modifier candidate under `XFR-D-021`, never a score/filter/Risk/Confidence/Qualification input and never automatically active; `FS-10` is `RESOLVED_V0_1_SCOPE_BOUNDARY`, with derived `budget_headroom` excluded from every v0.1 use while raw rent/budget fields, `budget_fit`, `rent_rate_fit`, both-max semantics and `XFR-D-013` remain unchanged; `FS-18` is `PARTIALLY_RESOLVED_BOUNDARY` for design-time-only `registry_readiness` with its four existing draft values unchanged, independent state axes, exact preservation of Architecture's seven-value evidence and six-value lawful-basis enums, candidate-token non-promotion and no-coercion/no-silent-cascade safeguards. The remaining exact derived formulas/numeric representations, ranking algorithms, future-re-entry contents, full/public runtime state enums, mappings, transitions, carriers, data, policies, production, runtime and implementation remain `OPEN`; no Proposal, Policy, Data Contract, dataset, evaluation, production, runtime or implementation is approved and all three gates remain `BLOCKED`.
...
Human-approved `XFR-D-015 v1.0` gives `FS-18` only `PARTIALLY_RESOLVED_BOUNDARY`: `registry_readiness` remains design-time only; `value_state`, exact seven-value `evidence_status` and `processing_eligibility` remain independent; the exact six-value Architecture §11 `lawful_basis_status` is preserved without additions or mappings. The draft tokens `PRESENT`/`NOT_APPLICABLE`/`UNKNOWN` and `ALLOWED`/`DATA_PROCESSING_BLOCKED` are not promoted to complete or public runtime enums. Missing/unknown/not-applicable/blocked/stale states cannot be coerced into another axis, zero, negative fact, `PASS`, `FAIL`, automatic `INELIGIBLE`, Qualification result or route. Full/public enums, mappings, transitions, cascade, granularity, schema, carrier, recovery and runtime remain `OPEN`.
...
Единственный writer `lawful_basis_id`, цели, версии, срока, отзыва и прекращения основания — существующий Lawful Basis/Consent Registry (Architecture §11, §21.3, §40.1). Human-approved `XFR-D-016 v1.0` даёт `FS-19` только `PARTIALLY_RESOLVED_BOUNDARY`: Matching Engine может позднее потреблять лишь purpose-bound, scope-compatible, versioned, read-only projection/ref с applicable source-owned invalidation и не может создавать, расширять, восстанавливать, заменять, изменять или переинтерпретировать lawful basis. Existing Reveal/Participation lawful-basis Data Contracts не образуют Matching contract и не импортируются по аналогии. Точный projection/contract representation и mapping, API/event/schema/carrier, version/hash/signature, TTL/cache/refresh/invalidation-delivery, RBAC/appointment, retry/recovery и operational consequences остаются `OPEN`; этот документ не создаёт контракт.

Architecture §11 source-normative `lawful_basis_status` enum остаётся ровно `ACTIVE | EXPIRED | REVOKED | TERMINATED | SUSPENDED | UNDER_REVIEW`. Для любого не-`ACTIVE` состояния, а также missing/unknown/stale/invalidated/version- or purpose-mismatched projection затронутое governed use fail closed без negative coercion, automatic `INELIGIBLE`, rejection, route или unrelated block. Это qualitative governance boundary, не новый/изменённый enum, не lawful-basis determination для feature/use и не runtime approval.
...
- **`required_evidence_level`** — будущая feature/source/use/purpose-specific sufficiency requirement относительно фактического `evidence_status` (§13 Architecture). `XFR-D-M1 v1.0` разрешает только qualitative governance/evidence boundary; exact level и assignment для **каждого** кандидата остаются `BLOCKED_PENDING_DECISION` и не утверждены ни для одного признака.
...
Поскольку `required_evidence_level` не утверждён ни для одного признака, **ни один** hard-constraint candidate этого draft не разрешает automatic `INELIGIBLE`. Отсутствие подтверждения может обрабатываться только по будущей separately approved `MATCHING_QUALIFICATION_POLICY`; exact `NEEDS_VERIFICATION`/`HUMAN_REVIEW_REQUIRED` applicability/route остаётся `OPEN` и не выводится автоматически из `XFR-D-M1`. §14 Architecture гарантирует только: «Если хотя бы одно условие не выполнено, Matching Engine не выставляет `INELIGIBLE`».
...
`rent_rate_fit` (№4) — **отдельно**: human-approved `XFR-D-003 v1.0` даёт только `PARTIALLY_RESOLVED_BOUNDARY` для representation governance derived `effective_rate = property_monthly_rent_rub ÷ property_area_sqm`. Raw CTA types/scales/units and the rule that rate maximum and total budget maximum both apply remain unchanged. A future representation contract must be explicit, immutable, version/hash-bound and attributable identically at the calculation and comparison checkpoints; missing/unknown/stale/incompatible contract blocks only the affected calculation without platform default, zero, `PASS`, `FAIL`, automatic `INELIGIBLE` or rejection. Exact derived type, scale, precision, rounding, checkpoints, equality/tolerance, conversion, serialization, edge/domain behavior and carrier remain `OPEN`/`BLOCKED_PENDING_DECISION`.
...
### 5.4. §14.3(4) — `XFR-D-014` governance/no-final-verdict boundary

Human-approved `XFR-D-014 v1.0` даёт `FS-17` только `PARTIALLY_RESOLVED_BOUNDARY`: governance owner — `LEGAL`, mandatory approvers — `Chief AI Architect + PRODUCT + AI + DEVELOPMENT`, evidence/technical-procedure owner — `AI + DEVELOPMENT` без unilateral authority. Условие 4 §14.3 Architecture («не protected attribute/proxy/discriminatory restriction») **не объявляется выполненным** ни для одной из 20 строк: record не выдаёт final LEGAL verdict, не классифицирует и не продвигает ни одного кандидата. Особое внимание для будущего item-specific LEGAL review:
...
Все 20 строк остаются `ELIGIBILITY_HARD_CONSTRAINT_CANDIDATE`; автоматический `INELIGIBLE` запрещён (`automatic_ineligible_allowed = NO`, §4.3, §5.2). Будущий item-specific verdict требует exact candidate/source/operator/use/version/hash binding, separate protected/proxy/discrimination classification, lawful basis, applicable compatibility и evidence-sufficiency determination. Confirmed protected/proxy означает unconditional exclusion; lawful basis не может снять этот запрет. Missing/unknown/unclassified/stale/conflicting evidence блокирует только promotion затронутого кандидата и не создаёт negative fact, failure, rejection или automatic `INELIGIBLE`. Exact verdicts, classifications, lawful basis, compatibility, evidence и downstream contents остаются `OPEN`.
...
| --- | --- | --- |
| `deal_priority` | `strategy_preferences.deal_priority` (`property_deal_priority` / `request_deal_priority`) | `RESOLVED_QUALITATIVE_BOUNDARY` under human-approved `XFR-D-006 v1.0`: Property and Request values remain separately attributable; any future use is only a separately governed ranking-modifier candidate under `XFR-D-021`, never a Dimension/Match/Priority Score, Hard Constraint, Eligibility, Risk, Confidence or Qualification input. No automatic order, route, rejection, disclosure or Campaign transition is authorized; activation, algorithm, sign/direction, weight, normalization, order/precedence/tie-break, candidate set, fallback and carrier remain `OPEN`. This preserves the source rule that `strategy_preferences` is a Campaign-management priority, not a Candidate filter |

### 6.4. Diagnostic-only / blocked / excluded soft candidates

| `feature_id` | Классификация | Обоснование |
| --- | --- | --- |
| `budget_headroom` | `registry_readiness = EXCLUDED_FROM_V0_1` — `RESOLVED_V0_1_SCOPE_BOUNDARY` | Human-approved `XFR-D-008 v1.0`: derived signal excluded from every v0.1 fit, score, rank, filter, Hard Constraint, Eligibility, Risk, Confidence and Qualification use. Raw rent/budget fields, existing `budget_fit`/`rent_rate_fit`, both-max semantics and `XFR-D-013` OPEX boundary remain unchanged. A maximum is not a cheaper-is-better preference; absence creates no zero, `PASS`/`FAIL`, Risk, route or rejection. Future re-entry and exact direction/formula/normalization/threshold remain `OPEN` |
...
| `business_stage_signal` | `registry_readiness = EXCLUDED_FROM_V0_1` — `RESOLVED_V0_1_SCOPE_BOUNDARY` | Human-approved `XFR-D-007 v1.0`: signal исключён из every v0.1 Matching use — fit/scoring, Risk, filtering, Hard Constraint, eligibility, Confidence, Qualification, ranking и proxy inference. Raw `request_business_stage` остаётся без изменений в существующем `subject_snapshot.business_stage` source mapping и не удаляется/не переименовывается/не реклассифицируется этим решением. Future re-entry требует separate versioned `LEGAL + PRODUCT` decision с then-approved protected/proxy classification и lawful basis under independent `XFR-D-054`, purpose и evidence; ни classification, ни lawful basis, ни future mechanism сейчас не утверждены |
...
| `feature_id` | Классификация | Обоснование |
| --- | --- | --- |
| `identity_authority_verification_status` | `GATE_ONLY_CANDIDATE` (не числовой Owner Fit input) — `PARTIALLY_RESOLVED_BOUNDARY` | Human-approved `XFR-D-004 v1.0`: Identity/Authority Registry — sole source; Matching Engine может позднее потреблять только purpose/scope-compatible immutable versioned read-only status/ref как gate-only input, никогда не верифицирует identity/authority, не получает raw documents/payloads и не использует факт как numeric Owner Fit, Match Score, Risk, ranking или Hard Constraint. Missing/unknown/stale/expired/revoked/invalidated/version-mismatched projection fail closed только для affected governed use без negative coercion, guessed failure, automatic `INELIGIBLE` или rejection. Exact status enum/projection/API/event/schema/carrier/freshness/TTL/evidence package/RBAC/operational consequences остаются `OPEN` |
...
| Identity/authority | 5. external gate status (candidate) | `XFR-D-004 v1.0` подтверждает только `PARTIALLY_RESOLVED_BOUNDARY` для qualitative source/read-only `GATE_ONLY` consumption: current purpose/scope-compatible immutable versioned projection/ref не может считаться текущей после applicable source status/version/invalidation/revocation event. Exact projection, delivery, cache/refresh, freshness/TTL и operational mechanics не определены (§7.1) | `BLOCKED_PENDING_DECISION` — exact mechanism |
...
Статус строк ниже не единообразно `open`: часть решений теперь `PARTIALLY RESOLVED`/`PARTIALLY_RESOLVED_BOUNDARY` (human-approved qualitative governance decision резолвит часть, remaining часть открыта), `RESOLVED_QUALITATIVE_BOUNDARY` (qualitative rule утверждена целиком, exact runtime representation остаётся отдельным open вопросом), `RESOLVED_V0_1_SCOPE_BOUNDARY` (v0.1-состав определён однозначно, future re-entry — отдельный downstream-вопрос) или `RESOLVED_QUALITATIVE_LITERAL_BASELINE` (qualitative comparison method утверждён целиком, normalization/alias/catalog-id enhancements остаются open) благодаря Wave 2B decision records `XFR-D-001`/`XFR-D-002`/`XFR-D-012`/`XFR-D-013 v1.0`, Wave 2C decision records `XFR-D-009`/`XFR-D-011 v1.0`, later `XFR-D-010 v1.0`, G1 legal-admissibility records `XFR-D-004`/`XFR-D-007`/`XFR-D-014`/`XFR-D-016 v1.0` and G2 feature-input-semantics records `XFR-D-003`/`XFR-D-006`/`XFR-D-008`/`XFR-D-015 v1.0`. Эти records — authority для соответствующей qualitative interpretation источника, не сам Feature Schema Proposal; ни одна строка не переводится в полностью закрытую этим документом.

| № | Открытое решение | Owner |
| --- | --- | --- |
| 1 | **`PARTIALLY_RESOLVED_BOUNDARY` — merged `XFR-D-M1 v1.0`.** Human-approved only: candidate-derived/non-source-normative governance owner and separate artifact/approver/evidence roles; exact seven-value `evidence_status` preservation; `input_validated`/parser/AI confidence do not establish evidence confirmation; separation from Evidence Confidence, Feature Fit, processing eligibility, Hard Constraint/Eligibility, overall Confidence, Risk and Qualification; closed version/hash-bound feature/source/use/purpose candidate discipline; affected-use-only fail-closed without defaults, negative coercion, automatic `INELIGIBLE`, route, reason or display; non-compensation/no-double-counting, qualitative evidence prerequisites, historical integrity and no automatic action. Every exact level/assignment/sufficiency mapping/order/hierarchy/default/fallback/freshness/conflict/expiry/revocation/reviewer/Qualification/data/policy/production/carrier/runtime/implementation content remains `OPEN` | Governance owner `PRODUCT + AI + Chief AI Architect` — human-approved candidate-derived, not `SOURCE_NORMATIVE`; Feature artifact owner `PRODUCT + LEGAL + AI`; Qualification artifact owner `Chief AI Architect + PRODUCT` under `XFR-D-030`; mandatory approvers `LEGAL + DEVELOPMENT`; evidence/technical-procedure owner `AI + DEVELOPMENT` without unilateral authority |
...
| 4 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-003 v1.0`.** Only the qualitative explicit immutable version/hash-bound representation discipline for derived `effective_rate` calculation and comparison is approved. Raw CTA types/scales/units and both-max semantics remain unchanged; missing/incompatible contract blocks only the affected calculation without platform default or business verdict. Exact type/scale/precision/rounding/checkpoints/equality/tolerance/conversion/serialization/edge behavior/carrier/runtime remain `OPEN` | Governance owner `DEVELOPMENT + AI` — human-approved candidate-derived, not `SOURCE_NORMATIVE`; mandatory approvers `Chief AI Architect + PRODUCT + LEGAL`; evidence/technical-procedure owner `DEVELOPMENT + AI` without unilateral authority |
| 5 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-004 v1.0`.** Identity/Authority Registry — sole source; future Matching consumption допускается только как purpose/scope-compatible immutable versioned read-only `GATE_ONLY` status/ref, без verification/raw documents/numeric Owner Fit/score/Risk/ranking/Hard Constraint use. Affected-use fail-closed boundary approved; exact status enum, projection, carrier, freshness/TTL, evidence package, appointment/RBAC and operational consequences remain `OPEN` | Governance owner `Chief AI Architect + LEGAL`; mandatory approvers `PRODUCT + AI + DEVELOPMENT`; evidence/technical-procedure owner `AI + DEVELOPMENT` without unilateral authority |
| 6 | Численный TTL для time-bound freshness (класс 3) | PRODUCT + AI, через `MATCHING_EVALUATION_PLAN` |
| 7 | **`PARTIALLY_RESOLVED_BOUNDARY` — merged `XFR-D-M3 v1.0` (`FS-07`, `PRIMARY_MERGED_MEMBER`).** Только role split, four-context separation, closed immutable version/hash-bound candidate discipline, affected-use fail-closed, non-compensation, source-normative deny preservation, historical binding, no-automatic-action и qualitative evidence prerequisites утверждены; exact method/cohort/quasi-ID/adversary/linkage/numerator/denominator/counting/threshold/comparator/tolerance/statistics/data/evidence/policy/production/schema/runtime/implementation остаются `OPEN` | `PRODUCT + LEGAL` — human-approved candidate-derived governance owner, не `SOURCE_NORMATIVE` для whole merge; Feature artifact owner `PRODUCT + LEGAL + AI`; mandatory approvers `Chief AI Architect + AI + DEVELOPMENT`; evidence/technical-procedure `AI + DEVELOPMENT` без unilateral authority |
| 8 | **`RESOLVED_QUALITATIVE_BOUNDARY` — `XFR-D-006 v1.0`.** Property and Request `deal_priority` remain separately attributable; future use is only a separately governed ranking-modifier candidate under `XFR-D-021`, never Dimension/Match/Priority Score, Hard Constraint, Eligibility, Risk, Confidence or Qualification input. No automatic ordering/routing/rejection/disclosure/Campaign transition is authorized. Activation, algorithm, sign/direction, weight, normalization, order/precedence/tie-break, candidate set, fallback and carrier remain `OPEN`; `XFR-D-024 v1.1` stays intact | Governance owner `AI + PRODUCT` — human-approved candidate-derived, not `SOURCE_NORMATIVE`; mandatory approvers `Chief AI Architect + LEGAL + DEVELOPMENT`; evidence/technical-procedure owner `AI + DEVELOPMENT` without unilateral authority |
| 9 | **`RESOLVED_V0_1_SCOPE_BOUNDARY` — `XFR-D-007 v1.0`.** `business_stage_signal` excluded from every v0.1 Matching use, включая fit/scoring/Risk/filtering/Hard Constraint/eligibility/Confidence/Qualification/ranking/proxy inference; raw `request_business_stage` source snapshot unchanged. Future re-entry and `XFR-D-054` protected/proxy classification/lawful basis remain separate `OPEN` work | Governance owner `LEGAL + PRODUCT`; mandatory approvers `Chief AI Architect + AI + DEVELOPMENT`; evidence/technical-procedure owner `AI + DEVELOPMENT` without unilateral authority |
| 10 | **`RESOLVED_V0_1_SCOPE_BOUNDARY` — `XFR-D-008 v1.0`.** Derived `budget_headroom` is `EXCLUDED_FROM_V0_1` for every fit/score/rank/filter/Hard Constraint/Eligibility/Risk/Confidence/Qualification use. Raw rent/budget fields, `budget_fit`, `rent_rate_fit`, both-max semantics and `XFR-D-013` remain unchanged; ceiling is not cheaper-is-better and absence creates no result. Future re-entry and exact direction/formula/normalization/threshold remain `OPEN` | Governance owner `PRODUCT + AI` — human-approved candidate-derived, not `SOURCE_NORMATIVE`; mandatory approvers `Chief AI Architect + LEGAL + DEVELOPMENT`; evidence/technical-procedure owner `AI + DEVELOPMENT` without unilateral authority |
...
| 12 | **`PARTIALLY_RESOLVED_BOUNDARY` — merged `XFR-D-M6 v1.0`.** Human-approved only: role/artifact-owner separation, semantic separation of Feature Fit, Feature Weight, categorical `evidence_status`, feature/value Evidence Confidence, `required_evidence_level`, overall Confidence, Hard Constraint/Eligibility, Risk and Qualification; exact seven-value Architecture §13 enum preservation; closed immutable version/hash-bound candidate discipline; affected-use fail-closed without numeric/default/negative coercion; explicit double-counting/non-compensation review, qualitative evidence prerequisites, historical binding and no automatic action. Every exact mapping/function/order/hierarchy/value/range/direction/monotonicity/default/fallback, `[0,1]` approval, normalization/denominator/missing arithmetic, data/metric/statistic/result, Policy/production/schema/carrier/runtime/implementation remains `OPEN` | Governance owner `Chief AI Architect + AI` — human-approved candidate-derived, not `SOURCE_NORMATIVE`; Feature artifact owner `PRODUCT + LEGAL + AI`; Scoring artifact owner `Chief AI Architect + PRODUCT`; Evaluation/evidence artifact owner `AI + DEVELOPMENT`; mandatory approvers `PRODUCT + LEGAL + DEVELOPMENT`; evidence/technical-procedure owner `AI + DEVELOPMENT` without unilateral authority |
...
| 17 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-014 v1.0`.** No final LEGAL verdict issued for any of 20 candidates; all remain `ELIGIBILITY_HARD_CONSTRAINT_CANDIDATE`, `automatic_ineligible_allowed = NO`. Confirmed protected/proxy means unconditional exclusion and lawful basis cannot override it; missing/unknown/unclassified/stale/conflicting evidence blocks promotion only. Exact verdicts/classifications/lawful basis/compatibility/evidence and downstream contents remain `OPEN` | Governance owner `LEGAL`; mandatory approvers `Chief AI Architect + PRODUCT + AI + DEVELOPMENT`; evidence/technical-procedure owner `AI + DEVELOPMENT` without unilateral authority |
| 18 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-015 v1.0`.** `registry_readiness` is design-time only; `value_state`, exact seven-value Architecture §13 `evidence_status` and `processing_eligibility` remain independent; exact six-value Architecture §11 `lawful_basis_status` remains unchanged. Draft state tokens are not full/public runtime enums; no coercion or silent cascade into another axis, zero, negative fact, `PASS`, `FAIL`, automatic `INELIGIBLE`, Qualification result or route. Full/public enums, mappings, transitions, cascade/granularity, schema, carrier, recovery and runtime remain `OPEN` | Governance owner `Chief AI Architect + DEVELOPMENT + AI` — human-approved candidate-derived, not `SOURCE_NORMATIVE`; mandatory approvers `PRODUCT + LEGAL`; evidence/technical-procedure owner `AI + DEVELOPMENT` without unilateral authority |
| 19 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-016 v1.0`.** Lawful Basis/Consent Registry — sole writer; Matching may later consume only a purpose-bound, scope-compatible, versioned, read-only projection/ref with applicable source-owned invalidation. Architecture §11 six-value enum remains unchanged; all non-`ACTIVE` and unavailable/incompatible states fail closed for affected use only. Existing Reveal/Participation contracts are not imported. Exact Matching contract/mapping/carrier/version-hash/TTL/RBAC/recovery/operations remain `OPEN` | Governance owner `Chief AI Architect + DEVELOPMENT + LEGAL`; mandatory approvers `PRODUCT + AI`; evidence/technical-procedure owner `AI + DEVELOPMENT` without unilateral authority |
...
| Категория | Элементы |
| --- | --- |
| `READY_FOR_DRAFT` | `FeatureValue` envelope §3; qualitative orthogonal state-axis model §3/§4 under `XFR-D-015 v1.0` (`PARTIALLY_RESOLVED_BOUNDARY`, full/public enums/mappings/transitions/carrier remain open); revision-инвалидация без исключений + same-revision calendar-time overlay для `property_available_from`/`request_move_in_by` (§8, скопировано дословно); exact evidence-status §13 and lawful-basis-status §11 Architecture enums as-is; boundary matrix §2; versioning-правила §9 |
| `READY_AS_CANDIDATE_ONLY` | Все 20 строк §5.1 остаются `ELIGIBILITY_HARD_CONSTRAINT_CANDIDATE` без final LEGAL verdict (`XFR-D-014 v1.0`) и с `automatic_ineligible_allowed = NO` (bootstrap mapping и operator-направление определены для всех; полный operator/mapping не определён для №8, 15, 20 — human-approved governance rules `XFR-D-001`/`XFR-D-002`/`XFR-D-012 v1.0` резолвят часть каждой (14/20 пар, baseline 3/16 пар, wildcard/land-правила соответственно), остаток см. `BLOCKED_PENDING_DECISION`; qualitative code-point literal-match метод для №17–19 утверждён целиком `XFR-D-011 v1.0` на design-time уровне — comparison-rule definition, не active runtime результат); `budget_fit`/`rent_rate_fit` basis-mismatch qualitative comparison rule (§5.3, `XFR-D-013 v1.0`) — design-time `value_state = UNKNOWN`/calculation-blocked candidate rule, без active scored результата; `rent_rate_fit` representation has only the `XFR-D-003 v1.0` qualitative version/hash discipline, with exact numeric contract blocked; `target_tenant_category_preference`; `location_priority_alignment` только для `parking`/`loading_access` (оба выводятся из Property-полей, не из geography, §7.4); revision-bound freshness как расширение на Matching Engine; coarse country/region/city/district geography **только** как hard-constraint candidates №16–19 (§7.4 — не source-поле `location_priority_alignment`); `combined_minimum_term_fact`; `utilities_included_fact` (informational); separately attributable `deal_priority` values only as a future ranking-modifier candidate under `XFR-D-006 v1.0`, with activation/algorithm blocked; `previous_contact_analysis_signal`; `identity_authority_verification_status` as `GATE_ONLY_CANDIDATE` under `XFR-D-004 v1.0`, with exact carrier/freshness/TTL still blocked |
| `BLOCKED_PENDING_DECISION` | Open rows №1 and №6 §10; exact/content portions of rows №4 (`XFR-D-003`), №5 (`XFR-D-004`), №17 (`XFR-D-014`), №18 (`XFR-D-015`) and №19 (`XFR-D-016`) remain blocked under their `PARTIALLY_RESOLVED_BOUNDARY`; row №8 is `RESOLVED_QUALITATIVE_BOUNDARY` only for `deal_priority` placement while activation/algorithm/ordering/carrier remain `OPEN`; rows №9 and №10 are `RESOLVED_V0_1_SCOPE_BOUNDARY` only for v0.1 exclusion, with future re-entry/content independently `OPEN`; row №7 / `XFR-D-M3` has only `PARTIALLY_RESOLVED_BOUNDARY`, with exact method/data/statistics blocked; №11/№14 are their recorded qualitative/scope boundaries; exact/content portions №12 after `XFR-D-M6` and №13 after `XFR-D-010` remain blocked; remaining unresolved portions №2/№3/№15 after Wave 2B, `security_deposit_fact`, required-evidence levels for all 20, every item-specific LEGAL verdict/classification/evidence package, derived `effective_rate` exact numeric contract, `deal_priority` activation/algorithm, `budget_headroom` future re-entry/content, full runtime state contract, Identity/Authority exact enum/projection/carrier/freshness/TTL/RBAC/operations, and Lawful Basis exact Matching contract/mapping/carrier/version-hash/TTL/RBAC/recovery/operations |
| `EXCLUDED_FROM_V0_1` | `min_lease_term_compatibility` as derived hard constraint (raw fields remain `combined_minimum_term_fact`); `area_centeredness`; separate `parking_preference_fit`/`access_mode_fit`; `business_stage_signal` under `XFR-D-007 v1.0` while raw `request_business_stage` remains unchanged; derived `budget_headroom` under `XFR-D-008 v1.0` while raw rent/budget fields and `budget_fit`/`rent_rate_fit` remain unchanged; derived `location_priority_alignment` for 8 of 10 `location_priority` values (`XFR-D-009 v1.0`, raw input preserved); `expected_occupancy_signal` (`XFR-D-009 v1.0`, raw input preserved); any raw identity/authority/previous-contact/payer/payment payload; `property_exact_address`/coordinates in scoring and presentation/telemetry/logs |

> **Temporal hygiene for row №1:** phrase «открытое решение №1 целиком» and bare `required_evidence_level` wording above reflect the pre-`XFR-D-M1` checkpoint. Current reading is `PARTIALLY_RESOLVED_BOUNDARY`: only qualitative governance/evidence safeguards are approved; every exact level, assignment, mapping and operational consequence remains `BLOCKED_PENDING_DECISION`.

> **Temporal hygiene for row №7:** historical wording that calls the re-identification method/threshold question wholly `OPEN` is superseded by the current `XFR-D-M3` overlay: only the qualitative boundary is `PARTIALLY_RESOLVED_BOUNDARY`; every exact method, threshold, data, evidence, policy, production and runtime content remains `OPEN`.

> **Temporal hygiene for G1 rows №5/№9/№17/№19:** pre-G1 wording that calls these rows wholly `OPEN`, treats `business_stage_signal` re-entry as presently reviewable, or treats Identity/Authority/Lawful Basis consumption as an approved exact contract is superseded only by `XFR-D-004`/`XFR-D-007`/`XFR-D-014`/`XFR-D-016 v1.0`. Current statuses are respectively `PARTIALLY_RESOLVED_BOUNDARY`, `RESOLVED_V0_1_SCOPE_BOUNDARY`, `PARTIALLY_RESOLVED_BOUNDARY`, `PARTIALLY_RESOLVED_BOUNDARY`; all exact/content/downstream items named in those records remain `OPEN`, and all gates remain `BLOCKED`.

> **Temporal hygiene for G2 rows №4/№8/№10/№18:** pre-G2 wording that calls these rows wholly `OPEN` is superseded only by `XFR-D-003`/`XFR-D-006`/`XFR-D-008`/`XFR-D-015 v1.0`. Current statuses are respectively `PARTIALLY_RESOLVED_BOUNDARY`, `RESOLVED_QUALITATIVE_BOUNDARY`, `RESOLVED_V0_1_SCOPE_BOUNDARY`, `PARTIALLY_RESOLVED_BOUNDARY`; exact numeric/algorithmic/future-re-entry/enum/mapping/carrier/runtime contents remain `OPEN`, and all gates remain `BLOCKED`.
...
`XFR-D-M1 v1.0` сохраняет эту границу: порядок семи статусов не создаёт hierarchy/promotion, а exact sufficiency mapping и per-feature/source/use/purpose assignment остаются `OPEN`.
...
**Then:** оба ограничения применяются одновременно (§5.1, №4), как того требует §8.4.8 источника; `XFR-D-003 v1.0` approves only the qualitative explicit immutable version/hash-bound representation discipline, while exact decimal precision/rounding and every other numeric/serialization detail for `effective_rate` remain `BLOCKED_PENDING_DECISION`/`OPEN` (§5.2), not defined by this document.
...
**Then:** ни один не найден; такие элементы явно отнесены к `MATCHING_SCORING_POLICY`/`MATCHING_RISK_POLICY`/`MATCHING_QUALIFICATION_POLICY` (§1.2, §2). `XFR-D-M6 v1.0` сохраняет это ограничение: он разрешает только qualitative governance/evidence boundary, а любая exact Feature Fit/Evidence Confidence mapping, function, numeric value/range (включая `[0,1]` как approved contract), calibration или runtime content остаётся `OPEN`.
...
#### `MFS-C-014` — `XFR-D-014`: no final LEGAL verdict по §14.3 условию 4
...
**Then:** по `XFR-D-014 v1.0` final LEGAL verdict не выдан ни для одного кандидата и условие не считается выполненным или проваленным; все 20 остаются `ELIGIBILITY_HARD_CONSTRAINT_CANDIDATE`, `automatic_ineligible_allowed = NO`. Future item-specific verdict требует separate version/hash-bound evidence-backed decision полного owner/approver set; confirmed protected/proxy means unconditional exclusion, while missing/unknown/unclassified/stale/conflicting evidence blocks promotion only.
...
#### `MFS-C-024` — Identity/authority source and `GATE_ONLY` boundary (`XFR-D-004`)

**Given:** `identity_authority_verification_status`. **When:** Matching use is considered. **Then:** Identity/Authority Registry remains the sole source and any future consumption is only purpose/scope-compatible, immutable, versioned and read-only as `GATE_ONLY`; Matching never verifies identity/authority, receives raw documents/payloads or uses the fact as numeric Owner Fit, Match Score, Risk, ranking or Hard Constraint result. Missing/unknown/stale/expired/revoked/invalidated/version-mismatched projection fails closed only for the affected governed use without negative coercion or automatic `INELIGIBLE`; exact enum/projection/carrier/freshness/TTL/evidence package/RBAC/operations remain `OPEN`.

#### `MFS-C-025` — `business_stage_signal` v0.1 exclusion (`XFR-D-007`)

**Given:** `business_stage_signal`. **When:** any v0.1 fit/scoring/Risk/filter/Hard Constraint/eligibility/Confidence/Qualification/ranking/proxy-inference use is attempted. **Then:** it remains `EXCLUDED_FROM_V0_1`, while raw `request_business_stage` remains unchanged in `subject_snapshot.business_stage`; no Matching result is produced. Future re-entry requires a separate versioned `LEGAL + PRODUCT` decision with approved protected/proxy classification and lawful basis under independent `XFR-D-054`, purpose and evidence.

#### `MFS-C-026` — no final LEGAL verdict for 20 candidates (`XFR-D-014`)

**Given:** any §5 candidate. **When:** approval, classification, promotion or automatic exclusion is requested from this sync. **Then:** no final LEGAL verdict exists; all 20 remain `ELIGIBILITY_HARD_CONSTRAINT_CANDIDATE`, `automatic_ineligible_allowed = NO`; confirmed protected/proxy means unconditional exclusion and cannot be overridden by lawful basis, while missing/unknown/unclassified/stale/conflicting evidence blocks promotion only and creates no negative fact. Exact verdicts/classifications/lawful basis/compatibility/evidence remain `OPEN`.

#### `MFS-C-027` — Lawful Basis Registry mechanism boundary (`XFR-D-016`)

**Given:** Architecture §11 lawful-basis fields and exact six-value `lawful_basis_status` enum. **When:** a Matching integration is considered. **Then:** Lawful Basis/Consent Registry remains the sole writer; future consumption is only purpose-bound, scope-compatible, versioned and read-only with applicable source-owned invalidation; every non-`ACTIVE` state and missing/unknown/stale/invalidated/version- or purpose-mismatched projection fails closed only for the affected governed use without automatic `INELIGIBLE`, rejection, route or unrelated block. Existing Reveal/Participation contracts are not imported; exact Matching contract/mapping/carrier/version-hash/TTL/RBAC/recovery/operations remain `OPEN`.

#### `MFS-C-028` — derived `effective_rate` representation contract remains exact-content open (`XFR-D-003`)

**Given:** `rent_rate_fit` derived `effective_rate`. **When:** calculation/comparison representation is reviewed. **Then:** a future contract must be explicit, immutable, version/hash-bound and identical at calculation/comparison checkpoints; raw CTA types/scales and both-max semantics remain unchanged. Missing/incompatible contract blocks only the affected calculation without platform default or business verdict; every exact numeric/rounding/serialization/edge/carrier detail remains `OPEN`.

#### `MFS-C-029` — `deal_priority` placement is qualitative only (`XFR-D-006`)

**Given:** separately attributable Property and Request `deal_priority`. **When:** a Matching use is proposed. **Then:** the only permitted future placement is a separately governed ranking-modifier candidate under `XFR-D-021`; it is never a Dimension/Match/Priority Score, Hard Constraint, Eligibility, Risk, Confidence or Qualification input and creates no automatic action. Activation and every exact algorithm/ordering/fallback/carrier detail remain `OPEN`; `XFR-D-024 v1.1` remains unchanged.

#### `MFS-C-030` — `budget_headroom` is excluded from v0.1 without deleting its raw inputs (`XFR-D-008`)

**Given:** derived `budget_headroom`. **When:** any v0.1 fit/score/rank/filter/Hard Constraint/Eligibility/Risk/Confidence/Qualification use is attempted. **Then:** the signal remains `EXCLUDED_FROM_V0_1`; raw rent/budget fields, `budget_fit`, `rent_rate_fit`, both-max semantics and `XFR-D-013` remain unchanged. A ceiling is not cheaper-is-better, absence produces no result, and future re-entry/content remains `OPEN`.

#### `MFS-C-031` — design-time readiness and runtime-relevant state axes remain distinct (`XFR-D-015`)

**Given:** `registry_readiness`, `value_state`, `evidence_status`, `processing_eligibility` and `lawful_basis_status`. **When:** their status contract is reviewed. **Then:** `registry_readiness` remains design-time only; the other three axes remain independent; Architecture's exact seven-value evidence and six-value lawful-basis enums remain unchanged; draft tokens are not full/public runtime enums. No state is coerced into another axis, zero, negative fact, `PASS`, `FAIL`, automatic `INELIGIBLE`, Qualification result or route; full enums/mappings/transitions/cascade/carrier/runtime remain `OPEN`.
...
- синхронизирует merged `XFR-D-M1 v1.0` только как `PARTIALLY_RESOLVED_BOUNDARY` для candidate-derived non-source-normative governance owner, separate Feature/Qualification artifact owners, mandatory approvers/evidence-role boundary, exact seven-value enum preservation, validation-not-confirmation, semantic separation, affected-use-only fail-closed, non-compensation/no-double-counting, immutable evidence, historical binding and no-automatic-action safeguards; every exact level/assignment/mapping/order/hierarchy/default/fallback/freshness/conflict/expiry/revocation/reviewer/Qualification/data/policy/production/schema/carrier/runtime/implementation content remains `OPEN`;
- синхронизирует merged `XFR-D-M6 v1.0` только как `PARTIALLY_RESOLVED_BOUNDARY` для candidate-derived non-source-normative governance owner, separate Feature/Scoring/Evaluation artifact owners, mandatory approvers/evidence-role boundary, semantic separation, exact seven-value enum preservation, affected-use fail-closed, double-counting/non-compensation review, immutable evidence, historical binding and no-automatic-action safeguards; every exact mapping/function/order/hierarchy/value/range/direction/default/fallback, `[0,1]` approval, normalization/denominator/missing arithmetic, data/metric/statistic/result, Policy/production/schema/carrier/runtime/implementation content remains `OPEN`;
- синхронизирует merged `XFR-D-M3 v1.0` только как `PARTIALLY_RESOLVED_BOUNDARY` для `FS-07 + EP-09 + MRP-10 + SPP-04`; roles/four-context separation, closed version/hash-bound candidate, affected-use fail-closed, non-compensation, source-normative denies, historical binding, no-automatic-action и qualitative evidence prerequisites approved; exact method/cohort/quasi-ID/adversary/linkage/numerator/denominator/counting/threshold/comparator/tolerance/statistics/data/evidence/policy/production/schema/runtime/implementation remain `OPEN`, all four source keys remain `PRIMARY_MERGED_MEMBER`, and all three gates remain `BLOCKED`;
- синхронизирует G1 legal-admissibility records `XFR-D-004`/`XFR-D-007`/`XFR-D-014`/`XFR-D-016 v1.0` только в пределах human-approved source/role/scope/no-verdict/fail-closed/non-conflation qualitative boundaries: statuses `PARTIALLY_RESOLVED_BOUNDARY` / `RESOLVED_V0_1_SCOPE_BOUNDARY` / `PARTIALLY_RESOLVED_BOUNDARY` / `PARTIALLY_RESOLVED_BOUNDARY`; canonical `FS-05`/`FS-09`/`FS-17`/`FS-19` identities and the 19-row register remain unchanged; exact carrier/status/TTL/evidence/classification/verdict/contract/RBAC/operational contents remain `OPEN`, no Proposal/Policy/Data Contract/dataset/evaluation/production/runtime/implementation is approved;
- синхронизирует G2 feature-input-semantics records `XFR-D-003`/`XFR-D-006`/`XFR-D-008`/`XFR-D-015 v1.0` только в пределах human-approved representation-discipline/ranking-placement/v0.1-exclusion/state-axis-separation qualitative boundaries: statuses `PARTIALLY_RESOLVED_BOUNDARY` / `RESOLVED_QUALITATIVE_BOUNDARY` / `RESOLVED_V0_1_SCOPE_BOUNDARY` / `PARTIALLY_RESOLVED_BOUNDARY`; canonical `FS-04`/`FS-08`/`FS-10`/`FS-18` identities and the 19-row register remain unchanged; raw CTA types/scales/rent/budget fields and both-max semantics remain unchanged, exact Architecture evidence/lawful-basis enums remain unchanged, and exact numeric/algorithmic/future-re-entry/enum/mapping/carrier/data/policy/production/runtime/implementation contents remain `OPEN`;
