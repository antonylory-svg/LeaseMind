...
Human-approved governance decisions `LeaseMind_MATCHING_DECISION_XFR-D-023_v1.0.md` (Scoring version compatibility/change rules), `LeaseMind_MATCHING_DECISION_XFR-D-026_v1.0.md` (synthetic-only vs production calibration boundary) и `LeaseMind_MATCHING_DECISION_XFR-D-028_v1.0.md` (Dimension Score explanation ownership boundary) — обязательные governance decisions для соответствующих граней ниже (§11, §12 строки №10/№15/№17, `MSP-C-007`/`MSP-C-010`/`MSP-C-019`); их approval не переводит этот Proposal в `APPROVED` и не утверждает numeric formula/threshold/runtime representation/field allowlist.

**Связанные документы:** `LeaseMind_MATCHING_ENGINE_ARCHITECTURE_v1.1.md` (полностью, включая §§5, 8–18, 24, 25, 30–38, 42, 49–53), `LeaseMind_MATCHING_DATA_CONTRACTS_v1.0.md` (прочитан полностью до EOF в рамках предыдущих задач этой сессии; независимо перепроверен на предмет scoring-специфичных терминов — см. §9 ниже), `LeaseMind_MATCHING_FEATURE_SCHEMA_v0.1.md` (Proposal-зависимость), `LeaseMind_MATCHING_EVALUATION_PLAN_v0.1.md` (Proposal-зависимость, threshold-search procedure), `LeaseMind_MATCHING_RISK_POLICY_v0.1.md` (Proposal-зависимость, Risk output boundary), `LeaseMind_MATCHING_QUALIFICATION_POLICY_v0.1.md` (Proposal-зависимость, routing boundary), `LeaseMind_MATCHING_DECISION_XFR-D-023_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-026_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-028_v1.0.md`, `02_PRODUCT/CAMPAIGN_TECHNICAL_ASSIGNMENT.md` (только как источник границы с отдельной pre-launch analysis системой — `deal_probability_30d`/Confidence Engine, не Matching Engine), `05_DEVELOPMENT/matching-engine/reviews/LeaseMind_DEVELOPMENT_REVIEW_MATCHING_ENGINE_v1.1_EIGHTH.md` (только DEVELOPMENT evidence о Data Contracts hash-верификации, не источник новых governance-решений).
...
Список не закрывается произвольно — ни одно решение не сокращено за счёт выдуманного значения. Пять из этих 18 строк (№10, №11, №15, №16, №17) синхронизированы с human-approved governance decisions `XFR-D-023`/`XFR-D-024`/`XFR-D-026`/`XFR-D-027`/`XFR-D-028 v1.0`, layered поверх этого Proposal — сам Proposal остаётся `Proposal for cross-functional review`, не `APPROVED`. №11 разрешён только в governance-owner половине; №16 разрешает только role boundary шагов §30.3 №1–3, оставляя exact procedure/quorum/content открытыми; №18 (`XFR-D-029`) уже задокументирован как anti-pattern guard, отдельного decision record не требует.
