...
Human-approved governance decisions `LeaseMind_MATCHING_DECISION_XFR-D-018_v1.0.md` (Scoring segment-override governance/evidence boundary), `LeaseMind_MATCHING_DECISION_XFR-D-021_v1.0.md` (ranking/diversification governance and qualitative-safeguard boundary), `LeaseMind_MATCHING_DECISION_XFR-D-023_v1.0.md` (Scoring version compatibility/change rules), `LeaseMind_MATCHING_DECISION_XFR-D-026_v1.0.md` (synthetic-only vs production calibration boundary) и `LeaseMind_MATCHING_DECISION_XFR-D-028_v1.0.md` (Dimension Score explanation ownership boundary) — обязательные governance decisions для соответствующих граней ниже (§6, §12 строки №4/№8/№10/№15/№17, `MSP-C-002`/`MSP-C-007`/`MSP-C-010`/`MSP-C-018`/`MSP-C-019`); их approval не переводит этот Proposal в `APPROVED` и не утверждает numeric formula/threshold/runtime representation/field allowlist.

**Связанные документы:** `LeaseMind_MATCHING_ENGINE_ARCHITECTURE_v1.1.md` (полностью, включая §§5, 8–18, 24, 25, 30–38, 42, 49–53), `LeaseMind_MATCHING_DATA_CONTRACTS_v1.0.md` (прочитан полностью до EOF в рамках предыдущих задач этой сессии; независимо перепроверен на предмет scoring-специфичных терминов — см. §9 ниже), `LeaseMind_MATCHING_FEATURE_SCHEMA_v0.1.md` (Proposal-зависимость), `LeaseMind_MATCHING_EVALUATION_PLAN_v0.1.md` (Proposal-зависимость, threshold-search procedure), `LeaseMind_MATCHING_RISK_POLICY_v0.1.md` (Proposal-зависимость, Risk output boundary), `LeaseMind_MATCHING_QUALIFICATION_POLICY_v0.1.md` (Proposal-зависимость, routing boundary), `LeaseMind_MATCHING_DECISION_XFR-D-018_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-021_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-023_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-026_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-028_v1.0.md`, `02_PRODUCT/CAMPAIGN_TECHNICAL_ASSIGNMENT.md` (только как источник границы с отдельной pre-launch analysis системой — `deal_probability_30d`/Confidence Engine, не Matching Engine), `05_DEVELOPMENT/matching-engine/reviews/LeaseMind_DEVELOPMENT_REVIEW_MATCHING_ENGINE_v1.1_EIGHTH.md` (только DEVELOPMENT evidence о Data Contracts hash-верификации, не источник новых governance-решений).
...
| Priority/Ranking signal | §15.6 («может использоваться отдельный Priority Score, учитывающий Match Score, Confidence Score и Risk Score»); §24 (Ранжирование и диверсификация) | Опциональный сигнал для внутреннего ранжирования, не для итогового Match Score | `SOURCE_NORMATIVE`, что он **опционален** и раздельно виден для аудита; governance owner Priority Score policy частично разрешён `XFR-D-024 v1.0`, а формула, веса, ranking algorithm, numeric thresholds и activation conditions остаются `OPEN` (§12, открытое решение №11). Отдельно от Priority Score, governance owner будущей **ranking/diversification policy** (шире §24, не тождественна Priority Score) частично разрешён `XFR-D-021 v1.0`: `Chief AI Architect + PRODUCT`, approvers `LEGAL + DEVELOPMENT`, consulted `AI`; exact algorithm, ordering, tie-break, candidate-set rule, `K`, metric, formula, weight, threshold, diversity definition и target остаются `OPEN` (§12, открытое решение №8) |
...
| 11 | **`PARTIALLY_RESOLVED_BOUNDARY` — `XFR-D-024 v1.0`.** Governance owner будущей Priority Score policy — `Chief AI Architect + PRODUCT`; утверждена только owner-boundary. §15.6 сохраняет Priority Score опциональным; формула, веса, ranking algorithm, numeric thresholds, activation conditions и runtime representation остаются `OPEN` | `Chief AI Architect + PRODUCT` (candidate governance assignment; mandatory approvers `LEGAL + DEVELOPMENT`, consulted `AI`) | Ranking explainability — содержательная policy и реализация остаются blocking; governance-owner половина больше не blocking |
...
Список не закрывается произвольно — ни одно решение не сокращено за счёт выдуманного значения. Семь из этих 18 строк (№4, №8, №10, №11, №15, №16, №17) синхронизированы с human-approved governance decisions `XFR-D-018`/`XFR-D-021`/`XFR-D-023`/`XFR-D-024`/`XFR-D-026`/`XFR-D-027`/`XFR-D-028 v1.0`, layered поверх этого Proposal — сам Proposal остаётся `Proposal for cross-functional review`, не `APPROVED`. №4 разрешён только как qualitative governance/evidence boundary, а actual segment policy/evidence/value/approval остаются `OPEN`; №8 разрешён только как ranking/diversification governance/qualitative-safeguard boundary, а exact algorithm/metric/target/data/production/runtime остаются `OPEN`; №11 разрешён только в governance-owner половине; №16 разрешает только role boundary шагов §30.3 №1–3, оставляя exact procedure/quorum/content открытыми; №18 (`XFR-D-029`) уже задокументирован как anti-pattern guard, отдельного decision record не требует.
...
**Given** любой расчёт Match или future ranking/diversification candidate. **When** проверяется разделение входов и downstream authority. **Then** Tenant Fit, Owner Fit, Deal Feasibility, Match Score, Confidence Score, Risk Score, Qualification status, freshness, readiness, negotiation gaps, deduplication и diversity objective остаются раздельно attributable; ни один не silently derived/collapsed/substituted другим. Human-approved `XFR-D-028 v1.0` сохраняет internal ownership Dimension Score components, а `XFR-D-021 v1.0` запрещает low-Confidence promotion, Risk masking и diversification до separately approved minimum quality; internal rank не создаёт catalog/disclosure authority и не меняет payer/legal/Qualification/Reveal права. Внешняя granularity/wording/disclosure остаётся `OPEN` под Safe Presentation Policy (`XFR-D-072`/`XFR-D-077`).
...
**Given** deterministic scoring path или future ranking/diversification candidate. **When** выполняется exact replay и evidence review. **Then** input hashes, component scores, ranking, reasons и final package hash идентичны (§49); несовпадение — severity-1 defect. `XFR-D-021 v1.0` дополнительно требует exact candidate/version/hash, frozen evidence, tuning/final isolation, compatible-only comparison, full reporting и separate-family non-compensation без automatic policy/value/runtime/gate action. `XFR-D-023 v1.0` сохраняет prospective-only supersession и non-exhaustive breaking baseline; exact version scheme и runtime bundle representation остаются `OPEN`.
...
**Given** evaluation run предлагает candidate weight/function/ranking/diversification policy. **When** оценивается approval. **Then** ни Mutual Aggregate, ни weights, ни algorithm/order/tie-break/candidate set/`K`/metric/formula/threshold/diversity target, ни production/runtime/gate status не меняются автоматически; требуется отдельный §30.3 process и same-version/hash review. `XFR-D-021 v1.0` разрешает только governance/qualitative safeguards and evidence prerequisites, а `XFR-D-026 v1.0` запрещает считать synthetic-only evidence production calibration/readiness; production evidence само по себе также не создаёт automatic approval.
