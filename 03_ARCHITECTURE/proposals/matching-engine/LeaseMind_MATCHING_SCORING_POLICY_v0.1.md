...
Human-approved governance decisions `LeaseMind_MATCHING_DECISION_XFR-D-018_v1.0.md` (Scoring segment-override governance/evidence boundary), `LeaseMind_MATCHING_DECISION_XFR-D-023_v1.0.md` (Scoring version compatibility/change rules), `LeaseMind_MATCHING_DECISION_XFR-D-026_v1.0.md` (synthetic-only vs production calibration boundary) и `LeaseMind_MATCHING_DECISION_XFR-D-028_v1.0.md` (Dimension Score explanation ownership boundary) — обязательные governance decisions для соответствующих граней ниже (§6, §12 строки №4/№10/№15/№17, `MSP-C-007`/`MSP-C-010`/`MSP-C-018`/`MSP-C-019`); их approval не переводит этот Proposal в `APPROVED` и не утверждает numeric formula/threshold/runtime representation/field allowlist.

**Связанные документы:** `LeaseMind_MATCHING_ENGINE_ARCHITECTURE_v1.1.md` (полностью, включая §§5, 8–18, 24, 25, 30–38, 42, 49–53), `LeaseMind_MATCHING_DATA_CONTRACTS_v1.0.md` (прочитан полностью до EOF в рамках предыдущих задач этой сессии; независимо перепроверен на предмет scoring-специфичных терминов — см. §9 ниже), `LeaseMind_MATCHING_FEATURE_SCHEMA_v0.1.md` (Proposal-зависимость), `LeaseMind_MATCHING_EVALUATION_PLAN_v0.1.md` (Proposal-зависимость, threshold-search procedure), `LeaseMind_MATCHING_RISK_POLICY_v0.1.md` (Proposal-зависимость, Risk output boundary), `LeaseMind_MATCHING_QUALIFICATION_POLICY_v0.1.md` (Proposal-зависимость, routing boundary), `LeaseMind_MATCHING_DECISION_XFR-D-018_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-023_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-026_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-028_v1.0.md`, `02_PRODUCT/CAMPAIGN_TECHNICAL_ASSIGNMENT.md` (только как источник границы с отдельной pre-launch analysis системой — `deal_probability_30d`/Confidence Engine, не Matching Engine), `05_DEVELOPMENT/matching-engine/reviews/LeaseMind_DEVELOPMENT_REVIEW_MATCHING_ENGINE_v1.1_EIGHTH.md` (только DEVELOPMENT evidence о Data Contracts hash-верификации, не источник новых governance-решений).
...
| Priority/Ranking signal | §15.6 («может использоваться отдельный Priority Score, учитывающий Match Score, Confidence Score и Risk Score»); §24 (Ранжирование и диверсификация) | Опциональный сигнал для внутреннего ранжирования, не для итогового Match Score | `SOURCE_NORMATIVE`, что он **опционален** и раздельно виден для аудита; governance owner будущей policy частично разрешён `XFR-D-024 v1.0`, а формула, веса, ranking algorithm, numeric thresholds и activation conditions остаются `OPEN` (§12, открытое решение №11) |
...
Список не закрывается произвольно — ни одно решение не сокращено за счёт выдуманного значения. Шесть из этих 18 строк (№4, №10, №11, №15, №16, №17) синхронизированы с human-approved governance decisions `XFR-D-018`/`XFR-D-023`/`XFR-D-024`/`XFR-D-026`/`XFR-D-027`/`XFR-D-028 v1.0`, layered поверх этого Proposal — сам Proposal остаётся `Proposal for cross-functional review`, не `APPROVED`. №4 разрешён только как qualitative governance/evidence boundary, а actual segment policy/evidence/value/approval остаются `OPEN`; №11 разрешён только в governance-owner половине; №16 разрешает только role boundary шагов §30.3 №1–3, оставляя exact procedure/quorum/content открытыми; №18 (`XFR-D-029`) уже задокументирован как anti-pattern guard, отдельного decision record не требует.
...
**Given** любой расчёт Match. **When** запрашиваются пять показателей. **Then** Tenant Fit, Owner Fit, Deal Feasibility, Confidence Score и Risk Score вычислены и сохранены раздельно (§14 этапы 5–6, §5 принцип 9); ни один не подменяет другой. Human-approved `XFR-D-028 v1.0`: Scoring Policy владеет existence/separateness/internal-интерпретацией Dimension Score компонент (Tenant Fit/Owner Fit/Deal Feasibility) как governance boundary; внешняя granularity/wording/disclosure этих компонент пользователю через Match Package/Safe Presentation остаётся `OPEN` под Safe Presentation Policy (`XFR-D-072`/`XFR-D-077`), не резолвится этим document'ом.
...
**Given** deterministic scoring path. **When** выполняется exact replay на тех же входах/версиях. **Then** input hashes, component scores, ranking, reasons и final package hash идентичны (§49, дословно); несовпадение — severity-1 defect, блокирует версию правил. Human-approved `XFR-D-023 v1.0`: supersession — только prospective, исторический Match Result не переписывается новой `scoring_policy_version`; breaking baseline (Mutual Aggregate function/measurement dimension/active weight) — non-exhaustive, fail-closed для остального; ни `major`/`minor`/`patch`, ни exact runtime bundle representation не утверждены — остаются `OPEN`.
...
**Given** evaluation run (Evaluation Plan, Proposal) успешно завершён и предлагает candidate weight/function. **When** оценивается статус approval. **Then** ни один статус (Mutual Aggregate approval, weights approval, gate status) не меняется автоматически; требуется отдельный девятишаговый процесс §30.3, включая review Chief AI Architect и согласование PRODUCT/LEGAL. Human-approved `XFR-D-026 v1.0`: evidence, произведённое исключительно из synthetic dataset categories, не устанавливает production calibration/readiness само по себе; это не означает обратного — production evidence само по себе также не создаёт automatic approval, тот же §30.3-процесс остаётся обязательным независимо от источника evidence.
