...
- не закрывает Architecture §37 вопрос №6 полностью — `XFR-D-072 v1.0` разрешает только field-allowlist governance/evidence `PARTIALLY_RESOLVED_BOUNDARY`, а `XFR-D-074 v1.0`, `XFR-D-075 v1.0` и `XFR-D-076 v1.0` — только отдельные geographic-generalization, combination-risk algorithm и successive-disclosure budget governance/evidence `PARTIALLY_RESOLVED_BOUNDARY`; actual allowlist/level/algorithm/budget unit/scope key/evidence/policy/runtime/implementation остаются `OPEN`;
