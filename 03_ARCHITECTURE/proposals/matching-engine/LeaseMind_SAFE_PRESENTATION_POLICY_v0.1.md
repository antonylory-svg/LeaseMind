...
**Given** документ существует на уровне draft со статусом `Proposal for cross-functional review — does not authorize implementation` (не `APPROVED`) и human-approved `XFR-D-072 v1.0`/`XFR-D-074 v1.0`. **When** запрашивается статус вопроса №6. **Then** governance/evidence-prerequisite boundary явно `PARTIALLY_RESOLVED_BOUNDARY` для field allowlist (`XFR-D-072`) и отдельно для geographic generalization (`XFR-D-074`), decision owner `PRODUCT + LEGAL` в обоих случаях, а actual allowlist/geographic level/evidence/policy/runtime/implementation остаются `OPEN`.
