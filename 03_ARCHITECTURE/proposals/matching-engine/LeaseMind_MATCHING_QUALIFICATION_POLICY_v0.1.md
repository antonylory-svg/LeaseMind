...
**Связанные документы:** `LeaseMind_MATCHING_ENGINE_ARCHITECTURE_v1.1.md`, `LeaseMind_MATCHING_DATA_CONTRACTS_v1.0.md` (прочитан полностью до EOF, 3976 строк — Qualification-специфичных carrier'ов не найдено, см. §5), `LeaseMind_MATCHING_FEATURE_SCHEMA_v0.1.md` (Proposal-зависимость, source facts/evidence), `LeaseMind_MATCHING_EVALUATION_PLAN_v0.1.md` (Proposal-зависимость, dataset/metric/threshold-search procedure evidence), `LeaseMind_MATCHING_RISK_POLICY_v0.1.md` (Proposal-зависимость, Risk output boundary), `LeaseMind_MATCHING_DECISION_XFR-D-030_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-031_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-032_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-033_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-037_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-038_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-040_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-043_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-044_v1.0.md`, `LeaseMind_MATCHING_DECISION_XFR-D-055_v1.0.md`, `02_PRODUCT/CAMPAIGN_OUTCOMES.md`, `02_PRODUCT/ANALYSIS_SNAPSHOT.md`, `05_DEVELOPMENT/matching-engine/reviews/LeaseMind_DEVELOPMENT_REVIEW_MATCHING_ENGINE_v1.1_EIGHTH.md` (только DEVELOPMENT evidence о Data Contracts hash/executable verification, не источник новых архитектурных решений).
...
Сам Proposal не выбирает policy values. Внешние human governance records разрешили owner assignment №1 и responsibility boundary части №2; qualitative semantics строк №3, №4, №10, №11, №13 и №18 разрешены `RESOLVED_QUALITATIVE_BOUNDARY` records `XFR-D-032/033/037/038/040/044` (§1), а строка №17 получила только `PARTIALLY_RESOLVED_BOUNDARY` по `XFR-D-043`. Отдельный canonical Risk Policy decision `MRP-12 → XFR-D-055` разрешает только `PARTIALLY_RESOLVED_BOUNDARY` interface governance/read-only/fail-closed semantics и не добавляет строку в этот 20-row register; row №9 `XFR-D-M2` identity/status не меняются. Exact runtime representation, numeric thresholds, Risk trigger/mapping, reason-code catalog values/order и остальные substantive вопросы остаются open.
...
**Given** несколько условий §18.1 нарушены одновременно. **When** формируется routing. **Then** сохранение всех причин и evidence references для audit/explanation утверждено `XFR-D-040` (§14, ранее кандидат №4, теперь `RESOLVED_QUALITATIVE_BOUNDARY`); primary reason выбирается детерминированно из причины, определившей route по precedence `XFR-D-033`; при нескольких причинах одного precedence-класса порядок зависит от будущего approved reason catalog — до его утверждения primary/user-facing reason не изобретается.
...
- не содержит ни одного численного mutual-fit/Confidence/completeness/Risk weight/threshold/TTL, ни одного runtime/algorithmic precedence/aggregation implementation, ни одного `§25.1 → Qualification result` mapping, ни одного public/runtime enum, field, event или reason-code каталога; шесть human-approved qualitative governance decisions (`XFR-D-032/033/037/038/040/044`) сохраняются, `XFR-D-043` разрешает только `PARTIALLY_RESOLVED_BOUNDARY` prospective supersession/immutable-history/current-compatible-bundle/fail-closed semantics, а `XFR-D-055` — только `PARTIALLY_RESOLVED_BOUNDARY` Risk→Qualification governance/read-only/fail-closed semantics; exact taxonomy/matrix/lifecycle/trigger/mapping, values/catalog order, runtime carrier и implementation остаются `OPEN`;
- отражает human governance assignment `XFR-D-030`, responsibility boundary `XFR-D-031`, шесть qualitative semantics decisions `XFR-D-032/033/037/038/040/044` и partial boundaries `XFR-D-043`/`XFR-D-055`, но не утверждает сам Proposal, sibling Proposals, `XFR-D-M2`, exact compatibility/mapping или runtime representation;
