...
Сам Proposal не выбирает policy values. Внешние human governance records разрешили owner assignment №1 и responsibility boundary части №2; qualitative semantics строк №3, №4, №10, №11, №13 и №18 разрешены `RESOLVED_QUALITATIVE_BOUNDARY` records `XFR-D-032/033/037/038/040/044` (§1), строки №12, №14, №16, №17 и №19 получили только `PARTIALLY_RESOLVED_BOUNDARY` по `XFR-D-039`, `XFR-D-041`, `XFR-D-042`, `XFR-D-043` и `XFR-D-045`, а строка №20 — только `RESOLVED_EVIDENCE_BOUNDARY` по `XFR-D-046`. Отдельный canonical Risk Policy decision `MRP-12 → XFR-D-055` разрешает только `PARTIALLY_RESOLVED_BOUNDARY` interface governance/read-only/fail-closed semantics и не добавляет строку в этот 20-row register; row №9 `XFR-D-M2` identity/status не меняются. Exact runtime representation, numeric thresholds, Risk trigger/mapping, reason-code catalog values/order, `XFR-F1` metric-family content, evidence/data/production contents и остальные substantive вопросы остаются open.
