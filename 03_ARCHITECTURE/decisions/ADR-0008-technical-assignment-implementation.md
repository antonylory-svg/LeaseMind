...
#### 1.2. Максимальная ставка TenantRequest для российского пилота

- `need_property` содержит 30-е поле
  `request_monthly_rent_rate_max_rub_per_sqm` (`numeric(12,2)`, nullable).
- Общий `request_monthly_budget_max_rub` остаётся обязательным и сохраняет
  обратную совместимость. При вводе ставки frontend рассчитывает его по
  максимальной площади, но ставка сохраняется отдельно: это не позволяет
  маленькому, но слишком дорогому за м² объекту пройти только по общему
  бюджету.
- Оба значения являются hard constraints. При режиме общей суммы ставка не
  задаётся и остаётся `null`.
...
- Изменение §3.1 (пользовательская последовательность), 30/30 утверждённых
